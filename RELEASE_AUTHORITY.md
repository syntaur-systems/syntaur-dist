...
## Fixed G1-G2-G3-G4-G5-G6-G7-G8-G9-G10 recovery

When the authority root is still absent after G10 has been published, use only
the versioned
`scripts/bootstrap-release-authority-g1-g2-g3-g4-g5-g6-g7-g8-g9-g10-recovery-v8.sh`.
Do not first install an earlier recovery. G10 is the controller authority; its
isolated build authority still reconstructs exact G2 source and exact Engine
source in separate worktrees.

Download the five immutable assets from each of `authority-v1-g1` through
`authority-v1-g10` into ten distinct canonical directories. Normalize
directories to `0500`, manifests and bundles to `0400`, and executables to
`0500`, then verify the complete signed successor chain:

```sh
scripts/bootstrap-release-authority-g1-g2-g3-g4-g5-g6-g7-g8-g9-g10-recovery-v8.sh verify \
  --g1-dir "$G1_DIR" \
  --g2-dir "$G2_DIR" \
  --g3-dir "$G3_DIR" \
  --g4-dir "$G4_DIR" \
...
With the exact G9 Genesis tools staged and the authority root still absent,
install the G10 provisioner and G10 shipper-as-validator:

```sh
sudo scripts/bootstrap-release-authority-g1-g2-g3-g4-g5-g6-g7-g8-g9-g10-recovery-v8.sh \
  stage-g10-build-authority \
  --g1-dir "$G1_DIR" \
  --g2-dir "$G2_DIR" \
  --g3-dir "$G3_DIR" \
  --g4-dir "$G4_DIR" \
...
The G9 and G10 provisioners are intentionally byte-identical; staging accepts
that shared exact digest without requiring a false binary distinction. The
G10 shipper-as-validator must have the distinct G10 digest.

Run the staged validator as the ordinary operator. The controller workspace
must be exact G10 commit
`247e46d23602803efb3703c6081198402978acb6`; the distinct baseline workspace
must be exact G2 commit
`5642d7bc36a4913d42d9bce1120a3a2fe604aca8`; and the Engine workspace must be
exact commit `36f3348fc32c02d0a0091be9ea87b828306941cc`:
...
env -i \
  ...
  PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
  LANG=C.UTF-8 LC_ALL=C.UTF-8 RUST_LOG=info \
  SYNTAUR_WORKSPACE="$G10_SOURCE_WORKTREE" \
  SYNTAUR_GENESIS_BASELINE_WORKSPACE="$G2_SOURCE_WORKTREE" \
  SYNTAUR_ENGINE_WORKSPACE="$GENESIS_ENGINE_WORKTREE" \
  /opt/syntaur-genesis-validator genesis-validate \
    --commit 247e46d23602803efb3703c6081198402978acb6 \
    --engine-commit 36f3348fc32c02d0a0091be9ea87b828306941cc \
    >"$GENESIS_EVIDENCE_PARTIAL" \
    2>"$GENESIS_LOG"
```

The canonical schema-v3 evidence binds G10 source and its exact G9 parent while
binding `baseline_source`, `build_authority.source_*`, and the persistent
catalog to G2. The browser audit reaches the G2 gateway through G10's fixed
loopback-only SSH tunnel, preserving the gateway bootstrap guard. Independently
record the evidence digest and current shipper digest, then install:
...
Installation atomically retains generations 1 through 10, activates G10,
installs the exact G10 shipper and provisioner, and records the G10 controller
...
## Fixed active-root G11-G12 recovery

Use this historical path only when the installed root is exact healthy G11 and
the immutable G12 publication is its direct signed successor. The predecessor
G10-G11 recovery must be terminal and free of journals, staging directories,
or input-snapshot transients. This is not a generic promotion, rollback, or
force path.

Verify the operator-owned, mode-`0500` G11 and G12 release directories first:

```sh
scripts/recover-release-authority-g11-g12-canary-root-v1.sh verify \
  --g11-dir "$G11_DIR" \
  --g12-dir "$G12_DIR"
```
...
```sh
sudo install -d -o root -g root -m 0700 \
  /etc/syntaur/release-authority-g11-g12-recovery-v1.runtime
sudo install -o root -g root -m 0500 \
  scripts/recover-release-authority-g11-g12-canary-root-v1.sh \
  scripts/release-authority-manifest.sh \
  /etc/syntaur/release-authority-g11-g12-recovery-v1.runtime/
sudo /etc/syntaur/release-authority-g11-g12-recovery-v1.runtime/\
recover-release-authority-g11-g12-canary-root-v1.sh install \
  --g11-dir "$G11_DIR" \
  --g12-dir "$G12_DIR" \
  --expected-current-shipper-sha256 \
    bfa75a5c7aa1af9a3d03cfd800d5221c6e8fe2717a697f1046245d8648793cc5
```
...
only generations 11 and 12 plus the manifest helper, while separately proving
the installed retained chain from generation 1 through 11. Before its first
...
G12 changes both live controller executables. Publication therefore proceeds
in this fixed order: retained generation 12, G12 shipper, G12 build-authority
provisioner, workflow trust, Cosign bundle, and active manifest last. The
root-owned journal records shipper and provisioner as distinct durable phases
and accepts only the exact phase state or its immediate crash-ahead state. The
active manifest cannot identify G12 until both executable replacements and the
signed trust material are durable.
...
12, the active copies, and both live executables are exact, it writes the fixed
receipt, retires the recovery journal, retires the sealed input snapshot, and
retires the shared mutation fence last. The completed G10-G11 receipt and
runtime are preserved as historical evidence.

Confirm the terminal state before using G12 for a product release:
...
## Fixed active-root G13-G14 recovery

Use this path only when the installed root is exact healthy G13 and the
immutable G14 publication is its direct signed successor. The predecessor
G12-G13 recovery must be terminal and free of journals, staging directories,
or input-snapshot transients. This is not a generic promotion, rollback, or
force path.

Verify the operator-owned, mode-`0500` G13 and G14 release directories first:

```sh
scripts/recover-release-authority-g13-g14-canary-root-v1.sh verify \
  --g13-dir "$G13_DIR" \
  --g14-dir "$G14_DIR"
```
...
```sh
sudo install -d -o root -g root -m 0700 \
  /etc/syntaur/release-authority-g13-g14-recovery-v1.runtime
sudo install -o root -g root -m 0500 \
  scripts/recover-release-authority-g13-g14-canary-root-v1.sh \
  scripts/release-authority-manifest.sh \
  /etc/syntaur/release-authority-g13-g14-recovery-v1.runtime/
sudo /etc/syntaur/release-authority-g13-g14-recovery-v1.runtime/\
recover-release-authority-g13-g14-canary-root-v1.sh install \
  --g13-dir "$G13_DIR" \
  --g14-dir "$G14_DIR" \
  --expected-current-shipper-sha256 \
    68f2017a1d5303b4cf3ee238b1b372c92ac0e3fc11fe284553e8845ce1b480ce
```

The recovery holds the root-promotion, global-mutation, and
operator-deployment locks for the whole transaction. It seals and revalidates
only generations 13 and 14 plus the manifest helper, while separately proving
the installed retained chain from generation 1 through 13. Before its first
authority mutation it publishes the fixed recovery fence at the shared normal
promotion-journal path. Re-run the identical sealed command after interruption;
never delete its fence, journal, or snapshot.

G14 changes the live shipper but retains the exact G13 build-authority
provisioner. Publication therefore proceeds in this fixed order: retained
generation 14, G14 shipper, the shared-provisioner checkpoint, workflow trust,
Cosign bundle, and active manifest last. The root-owned journal keeps the
provisioner checkpoint without rewriting the unchanged executable and accepts
only the exact phase state or its immediate crash-ahead state. The active
manifest cannot identify G14 until the shipper and signed trust material are
durable.

The script does not build, release, or deploy product bytes. It hashes the
bounded v0.7.114 and v0.7.115 product-release state before the transaction and
after both unprivileged `authority-status` checks. Once generations 1 through
14, the active copies, and both live executables are exact, it writes the fixed
receipt, retires the recovery journal, retires the sealed input snapshot, and
retires the shared mutation fence last. The completed G12-G13 receipt and
runtime are preserved as historical evidence.

Confirm the terminal state before using G14 for a product release:
...
## Fixed active-root G15-G16 recovery

Use this path only when the installed root is exact healthy G15 and the
immutable G16 publication is its direct signed successor. The predecessor
G14-G15 recovery must be terminal and free of journals, staging directories,
or input-snapshot transients. This is not a generic promotion, rollback, or
force path.

Verify the operator-owned, mode-`0500` G15 and G16 release directories first:

```sh
scripts/recover-release-authority-g15-g16-canary-root-v1.sh verify \
  --g15-dir "$G15_DIR" \
  --g16-dir "$G16_DIR"
```
...
```sh
sudo install -d -o root -g root -m 0700 \
  /etc/syntaur/release-authority-g15-g16-recovery-v1.runtime
sudo install -o root -g root -m 0500 \
  scripts/recover-release-authority-g15-g16-canary-root-v1.sh \
  scripts/release-authority-manifest.sh \
  /etc/syntaur/release-authority-g15-g16-recovery-v1.runtime/
sudo /etc/syntaur/release-authority-g15-g16-recovery-v1.runtime/\
recover-release-authority-g15-g16-canary-root-v1.sh install \
  --g15-dir "$G15_DIR" \
  --g16-dir "$G16_DIR" \
  --expected-current-shipper-sha256 \
    1bd103791f91e9980bda66d7079b287982bd26f72ab8fd556d27557bff9e12e7
```

The recovery holds the root-promotion, global-mutation, and
operator-deployment locks for the whole transaction. It seals and revalidates
only generations 15 and 16 plus the manifest helper, while separately proving
the installed retained chain from generation 1 through 15. Before its first
authority mutation it publishes the fixed recovery fence at the shared normal
promotion-journal path. Re-run the identical sealed command after interruption;
never delete its fence, journal, or snapshot.

G16 changes both the live shipper and verifier while retaining the exact G15
build-authority provisioner. Publication proceeds in this fixed order:
retained generation 16, G16 shipper, the shared-provisioner checkpoint,
workflow trust, Cosign bundle, and active manifest last. The root-owned journal
accepts only the exact phase state or its immediate crash-ahead state. The
active manifest cannot identify G16 until the shipper and signed trust material
are durable.

The script does not build, release, or deploy product bytes. It hashes the
bounded active v0.7.114 state and pending v0.7.126 release and source-pretag
state before the transaction and after both unprivileged `authority-status`
checks. Once generations 1 through 16, the active copies, and both live
executables are exact, it writes the fixed receipt, retires the recovery
journal, retires the sealed input snapshot, and retires the shared mutation
fence last. The completed G14-G15 receipt and runtime are preserved as
historical evidence.

Confirm the terminal state before using G16 for a product release:

```sh
/usr/local/bin/syntaur-ship authority-status
sha256sum /etc/syntaur/release-authority-g15-g16-recovery-v1.receipt.json
```
