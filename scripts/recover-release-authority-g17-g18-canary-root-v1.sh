...
readonly G17_MANIFEST_SHA256=258521d5589d794fb386a7c42a33e0a59124488937ad02beeba18ede981da445
readonly G17_BUNDLE_SHA256=e97f4f9e2c5edcf780f38162138a632870a6d86e4e0f40cd9e28d1297d72278b
readonly G17_WORKFLOW_COMMIT=fbe727dc36f744964d01da5be7b6a97a2ec2c185
readonly G17_SHIPPER_SHA256=106137d4a7fe6e113523c919ed0319af5878af72937062095efa11797660aa9a
readonly G17_VERIFIER_SHA256=defe6d60fd14b92adaa7eed6239da574de7122c11e5a9020bb7748718237255f
readonly G17_PROVISIONER_SHA256=315bf5591b1ac00ffb443a8e8003fb77b7a421a5e66e91e6e9fd3ed5f859860b

readonly G18_MANIFEST_SHA256=b3dc7007d54adf0a4b1337650bd681c70dd3b5036f7e7adb37c0e4843ebefb90
readonly G18_BUNDLE_SHA256=c0ffca9a52ac28fef78b8e1cac34b1cb124fd784fa85f724aeb4b2ade187c341
readonly G18_WORKFLOW_COMMIT=01e1ee9bddc5820a18356558ba03328871c96c93
readonly G18_SHIPPER_SHA256=1853745c0de4159dff779efc37c61f146fd9949132d8ffb213c2b64d699c08cc
readonly G18_VERIFIER_SHA256=67575018a8b9fa97498561ff8a1e81de9156d0c648257a4bc220c797a3d3b5f1
readonly G18_PROVISIONER_SHA256=315bf5591b1ac00ffb443a8e8003fb77b7a421a5e66e91e6e9fd3ed5f859860b
...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g17-g18-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-18-g17-g18-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g17-g18-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g17-g18-recovery-v1.runtime
...
die() {
    printf 'release authority G17-G18 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g17-g18-canary-root-v1.sh verify \
    --g17-dir DIR --g18-dir DIR

  sudo recover-release-authority-g17-g18-canary-root-v1.sh install \
    --g17-dir DIR --g18-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G17 authority root to
its exact immutable G18 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G18
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g17-g18-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g17-g18-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    local path
    for path in \
        "$AUTHORITY_ROOT/authority-g16-g17-recovery-v1.json" \
        "$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.json.tmp" \
        "$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.inputs" \
        "$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.inputs.staged" \
        "$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.inputs.retiring" \
        "$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.fence.tmp" \
        "$ARTIFACT_ROOT/.generation-17-g16-g17-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g16-g17-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G15-G17 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g17_dir=$1 g18_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G18_MANIFEST_SHA256" \
        || die 'G18 manifest digest was not fixed before publication'
    valid_sha256 "$G18_BUNDLE_SHA256" \
        || die 'G18 bundle digest was not fixed before publication'
    ...
    [[ $g17_dir != "$g18_dir" ]] || die 'G17 and G18 directories must differ'
    [[ $G17_SHIPPER_SHA256 != "$G18_SHIPPER_SHA256" ]] \
        || die 'G17 and G18 shipper digests must differ'
    [[ $G17_PROVISIONER_SHA256 == "$G18_PROVISIONER_SHA256" ]] \
        || die 'G17 and G18 must retain the exact shared provisioner'
    validate_material "$g17_dir" 17 \
        "$G17_MANIFEST_SHA256" "$G17_BUNDLE_SHA256" \
        "$G17_WORKFLOW_COMMIT" "$G17_SHIPPER_SHA256" \
        "$G17_VERIFIER_SHA256" "$G17_PROVISIONER_SHA256" \
        "$operator_uid" "$operator_gid" "$directory_mode" \
        "$data_mode" "$executable_mode"
    validate_material "$g18_dir" 18 \
        "$G18_MANIFEST_SHA256" "$G18_BUNDLE_SHA256" \
        "$G18_WORKFLOW_COMMIT" "$G18_SHIPPER_SHA256" \
        "$G18_VERIFIER_SHA256" "$G18_PROVISIONER_SHA256" \
        "$operator_uid" "$operator_gid" "$directory_mode" \
        "$data_mode" "$executable_mode"
    "$manifest_helper" assert-successor \
        "$g17_dir/release-authority-v2.json" \
        "$g18_dir/release-authority-v2.json"
}
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-17|generation-18|release-authority-manifest.sh) ;;
            *) die 'partial recovery input snapshot has an unexpected entry' ;;
        esac
    done <<<"$actual"
    for generation in 17 18; do
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-17" "$root/generation-18" 0 0 700 400 500
}

require_sources_match_snapshot() {
    local g17_dir=$1 g18_dir=$2 operator_uid=$3 operator_gid=$4
    local generation source name
    validate_material_layout "$g17_dir" 17 \
        "$operator_uid" "$operator_gid" 500 400 500
    validate_material_layout "$g18_dir" 18 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 17 18; do
        if [[ $generation == 17 ]]; then source=$g17_dir; else source=$g18_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g17_dir" "$g18_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
    ...
    /usr/bin/install -d -o root -g root -m 0700 \
        "$RECOVERY_SNAPSHOT_STAGE" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-17" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-18"
    snapshot_release "$g17_dir" "$RECOVERY_SNAPSHOT_STAGE/generation-17"
    snapshot_release "$g18_dir" "$RECOVERY_SNAPSHOT_STAGE/generation-18"
...
validate_complete_installed_g18() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_fixed_installed_generation 17 \
        "$G17_MANIFEST_SHA256" "$G17_BUNDLE_SHA256" \
        "$G17_WORKFLOW_COMMIT" "$G17_SHIPPER_SHA256" \
        "$G17_VERIFIER_SHA256" "$G17_PROVISIONER_SHA256"
    validate_fixed_installed_generation 18 \
        "$G18_MANIFEST_SHA256" "$G18_BUNDLE_SHA256" \
        "$G18_WORKFLOW_COMMIT" "$G18_SHIPPER_SHA256" \
        "$G18_VERIFIER_SHA256" "$G18_PROVISIONER_SHA256"
    validate_retained_chain 18
    validate_active_copy 18 "$ARTIFACT_ROOT/generation-18" \
        "$G18_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G18_SHIPPER_SHA256" 1755 || die 'final G18 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G18_PROVISIONER_SHA256" 755 || die 'final G18 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g17_sha=$3 g18_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g17_sha" ]]; then
        /usr/bin/printf '17\n'
    elif [[ $digest == "$g18_sha" ]]; then
        /usr/bin/printf '18\n'
    else
...
validate_phase_state() {
    ...
    local manifest_state vector g17_workflow_sha g18_workflow_sha
    validate_installed_generation 17 \
        "$RECOVERY_SNAPSHOT/generation-17" "$G17_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-18 || -L $ARTIFACT_ROOT/generation-18 ]]; then
        validate_installed_generation 18 \
            "$RECOVERY_SNAPSHOT/generation-18" "$G18_WORKFLOW_COMMIT"
        generation_state=18
    else
        generation_state=0
    fi
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G17_SHIPPER_SHA256" \
        "$G18_SHIPPER_SHA256" 'installed shipper')
    g17_workflow_sha=$(workflow_file_sha256 "$G17_WORKFLOW_COMMIT")
    g18_workflow_sha=$(workflow_file_sha256 "$G18_WORKFLOW_COMMIT")
    trust_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/trusted-workflow-commit" 444 \
        "$g17_workflow_sha" "$g18_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G17_BUNDLE_SHA256" "$G18_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G17_MANIFEST_SHA256" "$G18_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:17:17:17:17|prepared:18:17:17:17:17|\
        generation_published:18:17:17:17:17|\
        generation_published:18:18:17:17:17|\
        shipper_published:18:18:17:17:17|\
        provisioner_published:18:18:17:17:17|\
        provisioner_published:18:18:18:17:17|\
        trust_published:18:18:18:17:17|\
        trust_published:18:18:18:18:17|\
        bundle_published:18:18:18:18:17|\
        bundle_published:18:18:18:18:18|\
        manifest_published:18:18:18:18:18) ;;
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g17-g18-normal-mutation-fence.v1 \
        --arg recovery_journal authority-g17-g18-recovery-v1.json \
        --arg g17_manifest "$G17_MANIFEST_SHA256" \
        --arg g17_shipper "$G17_SHIPPER_SHA256" \
        --arg g17_provisioner "$G17_PROVISIONER_SHA256" \
        --arg g18_manifest "$G18_MANIFEST_SHA256" \
        --arg g18_bundle "$G18_BUNDLE_SHA256" \
        --arg g18_workflow "$G18_WORKFLOW_COMMIT" \
        --arg g18_shipper "$G18_SHIPPER_SHA256" \
        --arg g18_provisioner "$G18_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,normal_mutations_blocked:true,
          recovery_journal:$recovery_journal,
          previous_generation:17,previous_manifest_sha256:$g17_manifest,
          previous_shipper_sha256:$g17_shipper,
          previous_provisioner_sha256:$g17_provisioner,
          target_generation:18,target_manifest_sha256:$g18_manifest,
          target_bundle_sha256:$g18_bundle,
          target_workflow_commit:$g18_workflow,
          target_shipper_sha256:$g18_shipper,
          target_provisioner_sha256:$g18_provisioner,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g17-g18-recovery.v1 \
        --arg phase "$phase" \
        --arg g17_manifest "$G17_MANIFEST_SHA256" \
        --arg g17_shipper "$G17_SHIPPER_SHA256" \
        --arg g17_provisioner "$G17_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g17-g18-recovery-receipt.v1 \
        --arg previous_manifest "$G17_MANIFEST_SHA256" \
        --arg target_manifest "$G18_MANIFEST_SHA256" \
        --arg target_bundle "$G18_BUNDLE_SHA256" \
        --arg target_workflow "$G18_WORKFLOW_COMMIT" \
        --arg target_shipper "$G18_SHIPPER_SHA256" \
        --arg target_provisioner "$G18_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,completed:true,
          previous_generation:17,previous_manifest_sha256:$previous_manifest,
          target_generation:18,target_manifest_sha256:$target_manifest,
...
publish_generation_17() {
    local g18_dir=$1 destination=$ARTIFACT_ROOT/generation-18
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 18 "$g18_dir" "$G18_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g18_dir/release-authority-v2.json" \
        "$g18_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g18_dir/syntaur-build-authority-provision" \
        "$g18_dir/syntaur-ship-linux-x86_64" \
        "$g18_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G18_WORKFLOW_COMMIT" \
...
[[ -n $g17_dir && -n $g18_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g17_dir" "$g18_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G17-G18 signed successor verified: generation=18 manifest_sha256=%s\n' \
        "$G18_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G17_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G17'
...
if [[ $receipt_present == true \
    && $active_manifest == "$G18_MANIFEST_SHA256" ]] \
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G18_SHIPPER_SHA256" 1755 \
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G18_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'G18 authority status changed product or deployment state'
    assert_locks "$root_lock_identity" "$global_lock_identity" \
        "$deploy_lock_identity"
    if [[ $fence_present == true ]]; then
        preflight_recovery_temporaries
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G17-G18 authority recovery already installed: active_generation=18 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G18_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G18 is not complete'
preflight_recovery_temporaries
...
prepare_recovery_snapshot \
    "$g17_dir" "$g18_dir" "$operator_uid" "$operator_gid" "$snapshot_resume"
g17_material=$RECOVERY_SNAPSHOT/generation-17
g18_material=$RECOVERY_SNAPSHOT/generation-18
installed_executable_is_exact "$INSTALLED_PROVISIONER" \
    "$G18_PROVISIONER_SHA256" 755 \
    || die 'G17/G18 shared provisioner differs before recovery'
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G18_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G18_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G18 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G17_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G17 nor complete G18'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G17_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G17 nor complete G18'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G17_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G17'
    validate_installed_generation 17 "$g17_material" "$G17_WORKFLOW_COMMIT"
    validate_retained_chain 17
    validate_active_copy 17 "$g17_material" "$G17_WORKFLOW_COMMIT"
    run_operator_authority_status "$operator_uid" "$operator_gid"
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'G17 authority status changed product or deployment state'
    assert_locks "$root_lock_identity" "$global_lock_identity" \
        "$deploy_lock_identity"
...
assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_generation_17 "$g18_material"
write_journal generation_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-18/syntaur-ship-linux-x86_64" \
    "$INSTALLED_SHIPPER" "$G18_SHIPPER_SHA256" 1755 \
    "$G17_SHIPPER_SHA256" 1755
...
g18_workflow_sha=$(workflow_file_sha256 "$G18_WORKFLOW_COMMIT")
publish_exact_file \
    "$ARTIFACT_ROOT/generation-18/trusted-workflow-commit" \
    "$AUTHORITY_ROOT/trusted-workflow-commit" \
    "$g18_workflow_sha" 444 "$g17_workflow_sha" 444
write_journal trust_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-18/release-authority-v2.json.cosign.bundle" \
    "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
    "$G18_BUNDLE_SHA256" 444 "$G17_BUNDLE_SHA256" 444
write_journal bundle_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_exact_file \
    "$ARTIFACT_ROOT/generation-18/release-authority-v2.json" \
    "$AUTHORITY_ROOT/release-authority-v2.json" \
    "$G18_MANIFEST_SHA256" 444 "$G17_MANIFEST_SHA256" 444
write_journal manifest_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
validate_installed_generation 17 "$g17_material" "$G17_WORKFLOW_COMMIT"
validate_installed_generation 18 "$g18_material" "$G18_WORKFLOW_COMMIT"
validate_retained_chain 18
validate_active_copy 18 "$g18_material" "$G18_WORKFLOW_COMMIT"
installed_executable_is_exact "$INSTALLED_SHIPPER" \
    "$G18_SHIPPER_SHA256" 1755 || die 'final G18 shipper differs'
installed_executable_is_exact "$INSTALLED_PROVISIONER" \
    "$G18_PROVISIONER_SHA256" 755 || die 'final G18 provisioner differs'
...
/usr/bin/printf 'G17-G18 authority recovery installed: active_generation=18 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G18_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
