...
readonly MANIFEST_HELPER_SHA256=39c4161dfd650a54b33409ab39409665a783cb481f585b51c2273265525263d3

readonly G21_MANIFEST_SHA256=890ef1b74027f0d3bcdf8ed1b6f18da910d291a41847adbc0ec452c572bab983
readonly G21_BUNDLE_SHA256=e83c1700dc85443f7536ead79ed271f93df277c9383cea4cbd156e7ac3d21ff1
readonly G21_WORKFLOW_COMMIT=0198256efba74933566d4ebbc67427a6d492236c
readonly G21_SHIPPER_SHA256=80a06c82c2591f7a22448a1959d8fe19e870d364a052d9aed390b2e70cab0fe1
readonly G21_VERIFIER_SHA256=fbcf2efbd7a10c6b57e13e0f58bd419777034ee2835b027a73074b90ab83c9b2
readonly G21_PROVISIONER_SHA256=315bf5591b1ac00ffb443a8e8003fb77b7a421a5e66e91e6e9fd3ed5f859860b

readonly G22_MANIFEST_SHA256=74cbd8ac08b6260195abbb45100ad866836369a6774967eff1036880b54800b3
readonly G22_BUNDLE_SHA256=42b99afecb07eef70bb91391aa56ac03b67bf57603bf3c5ed22f49ff7625bcfb
readonly G22_WORKFLOW_COMMIT=0198256efba74933566d4ebbc67427a6d492236c
readonly G22_SHIPPER_SHA256=1bcdc22ba1b672745642ef7b927fce593b6e267627087943e2e2bafafc351536
readonly G22_VERIFIER_SHA256=fbcf2efbd7a10c6b57e13e0f58bd419777034ee2835b027a73074b90ab83c9b2
readonly G22_PROVISIONER_SHA256=315bf5591b1ac00ffb443a8e8003fb77b7a421a5e66e91e6e9fd3ed5f859860b
...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g21-g22-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g21-g22-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-22-g21-g22-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g21-g22-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g21-g22-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g21-g22-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g21-g22-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g21-g22-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g21-g22-recovery-v1.runtime
...
die() {
    printf 'release authority G21-G22 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g21-g22-canary-root-v1.sh verify \
    --g21-dir DIR --g22-dir DIR

  sudo recover-release-authority-g21-g22-canary-root-v1.sh install \
    --g21-dir DIR --g22-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G21 authority root to
its exact immutable G22 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G22
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g21-g22-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g21-g22-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    local path
    for path in \
        "$AUTHORITY_ROOT/authority-g20-g21-recovery-v1.json" \
        "$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.json.tmp" \
        "$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.inputs" \
        "$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.inputs.staged" \
        "$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.inputs.retiring" \
        "$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.fence.tmp" \
        "$ARTIFACT_ROOT/.generation-21-g20-g21-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g20-g21-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G15-G21 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g21_dir=$1 g22_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G22_MANIFEST_SHA256" \
        || die 'G22 manifest digest was not fixed before publication'
    valid_sha256 "$G22_BUNDLE_SHA256" \
        || die 'G22 bundle digest was not fixed before publication'
    ...
    [[ $g21_dir != "$g22_dir" ]] || die 'G21 and G22 directories must differ'
    [[ $G21_SHIPPER_SHA256 != "$G22_SHIPPER_SHA256" ]] \
        || die 'G21 and G22 shipper digests must differ'
    [[ $G21_PROVISIONER_SHA256 == "$G22_PROVISIONER_SHA256" ]] \
        || die 'G21 and G22 must retain the exact shared provisioner'
    validate_material "$g21_dir" 21 \
        "$G21_MANIFEST_SHA256" "$G21_BUNDLE_SHA256" \
        "$G21_WORKFLOW_COMMIT" "$G21_SHIPPER_SHA256" \
        "$G21_VERIFIER_SHA256" "$G21_PROVISIONER_SHA256" \
        "$operator_uid" "$operator_gid" "$directory_mode" \
        "$data_mode" "$executable_mode"
    validate_material "$g22_dir" 22 \
        "$G22_MANIFEST_SHA256" "$G22_BUNDLE_SHA256" \
        "$G22_WORKFLOW_COMMIT" "$G22_SHIPPER_SHA256" \
        "$G22_VERIFIER_SHA256" "$G22_PROVISIONER_SHA256" \
        "$operator_uid" "$operator_gid" "$directory_mode" \
        "$data_mode" "$executable_mode"
    "$manifest_helper" assert-successor \
        "$g21_dir/release-authority-v2.json" \
        "$g22_dir/release-authority-v2.json"
}
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-21|generation-22|release-authority-manifest.sh) ;;
            *) die 'partial recovery input snapshot has an unexpected entry' ;;
        esac
    done <<<"$actual"
    for generation in 21 22; do
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-21" "$root/generation-22" 0 0 700 400 500
}

require_sources_match_snapshot() {
    local g21_dir=$1 g22_dir=$2 operator_uid=$3 operator_gid=$4
    local generation source name
    validate_material_layout "$g21_dir" 21 \
        "$operator_uid" "$operator_gid" 500 400 500
    validate_material_layout "$g22_dir" 22 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 21 22; do
        if [[ $generation == 21 ]]; then source=$g21_dir; else source=$g22_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g21_dir" "$g22_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
    ...
    /usr/bin/install -d -o root -g root -m 0700 \
        "$RECOVERY_SNAPSHOT_STAGE" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-21" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-22"
    snapshot_release "$g21_dir" "$RECOVERY_SNAPSHOT_STAGE/generation-21"
    snapshot_release "$g22_dir" "$RECOVERY_SNAPSHOT_STAGE/generation-22"
...
validate_complete_installed_g22() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_fixed_installed_generation 21 \
        "$G21_MANIFEST_SHA256" "$G21_BUNDLE_SHA256" \
        "$G21_WORKFLOW_COMMIT" "$G21_SHIPPER_SHA256" \
        "$G21_VERIFIER_SHA256" "$G21_PROVISIONER_SHA256"
    validate_fixed_installed_generation 22 \
        "$G22_MANIFEST_SHA256" "$G22_BUNDLE_SHA256" \
        "$G22_WORKFLOW_COMMIT" "$G22_SHIPPER_SHA256" \
        "$G22_VERIFIER_SHA256" "$G22_PROVISIONER_SHA256"
    validate_retained_chain 22
    validate_active_copy 22 "$ARTIFACT_ROOT/generation-22" \
        "$G22_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G22_SHIPPER_SHA256" 1755 || die 'final G22 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G22_PROVISIONER_SHA256" 755 || die 'final G22 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g21_sha=$3 g22_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g21_sha" ]]; then
        /usr/bin/printf '21\n'
    elif [[ $digest == "$g22_sha" ]]; then
        /usr/bin/printf '22\n'
    else
...
validate_phase_state() {
    ...
    local manifest_state vector g21_workflow_sha g22_workflow_sha
    validate_installed_generation 21 \
        "$RECOVERY_SNAPSHOT/generation-21" "$G21_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-22 || -L $ARTIFACT_ROOT/generation-22 ]]; then
        validate_installed_generation 22 \
            "$RECOVERY_SNAPSHOT/generation-22" "$G22_WORKFLOW_COMMIT"
        generation_state=22
    else
        generation_state=0
    fi
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G21_SHIPPER_SHA256" \
        "$G22_SHIPPER_SHA256" 'installed shipper')
    g21_workflow_sha=$(workflow_file_sha256 "$G21_WORKFLOW_COMMIT")
    g22_workflow_sha=$(workflow_file_sha256 "$G22_WORKFLOW_COMMIT")
    trust_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/trusted-workflow-commit" 444 \
        "$g21_workflow_sha" "$g22_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G21_BUNDLE_SHA256" "$G22_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G21_MANIFEST_SHA256" "$G22_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:21:21:21:21|prepared:22:21:21:21:21|\
        generation_published:22:21:21:21:21|\
        generation_published:22:22:21:21:21|\
        shipper_published:22:22:21:21:21|\
        provisioner_published:22:22:21:21:21|\
        trust_published:22:22:21:21:21|\
        trust_published:22:22:22:21:21|\
        trust_published:22:22:21:22:21|\
        bundle_published:22:22:22:21:21|\
        bundle_published:22:22:22:22:21|\
        bundle_published:22:22:21:22:21|\
        bundle_published:22:22:21:22:22|\
        manifest_published:22:22:22:22:22|\
        manifest_published:22:22:21:22:22) ;;
...
product_state_digest() {
    ...
    local json_sha bundle_sha alias_target
    {
        ...
        for path in \
            "$OPERATOR_STATE/release-intent/v0.7.129.json" \
            "$OPERATOR_STATE/release-dispatch/v0.7.129.json" \
            "$OPERATOR_STATE/release-run/v0.7.129.json" \
            "$OPERATOR_STATE/release-authority/v0.7.129.json" \
            "$OPERATOR_STATE/source-preflight-dispatch/v0.7.129.json" \
            "$OPERATOR_STATE/source-preflight-run/v0.7.129.json"; do
            if [[ -f $path && ! -L $path ]]; then
                /usr/bin/printf 'present\0%s\0%s\0' "$path" "$(sha256_file "$path")"
            elif [[ ! -e $path && ! -L $path ]]; then
                /usr/bin/printf 'absent\0%s\0' "$path"
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g21-g22-normal-mutation-fence.v1 \
        --arg recovery_journal authority-g21-g22-recovery-v1.json \
        --arg g21_manifest "$G21_MANIFEST_SHA256" \
        --arg g21_shipper "$G21_SHIPPER_SHA256" \
        --arg g21_provisioner "$G21_PROVISIONER_SHA256" \
        --arg g22_manifest "$G22_MANIFEST_SHA256" \
        --arg g22_bundle "$G22_BUNDLE_SHA256" \
        --arg g22_workflow "$G22_WORKFLOW_COMMIT" \
        --arg g22_shipper "$G22_SHIPPER_SHA256" \
        --arg g22_provisioner "$G22_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,normal_mutations_blocked:true,
          recovery_journal:$recovery_journal,
          previous_generation:21,previous_manifest_sha256:$g21_manifest,
          previous_shipper_sha256:$g21_shipper,
          previous_provisioner_sha256:$g21_provisioner,
          target_generation:22,target_manifest_sha256:$g22_manifest,
          target_bundle_sha256:$g22_bundle,
          target_workflow_commit:$g22_workflow,
          target_shipper_sha256:$g22_shipper,
          target_provisioner_sha256:$g22_provisioner,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g21-g22-recovery.v1 \
        --arg phase "$phase" \
        --arg g21_manifest "$G21_MANIFEST_SHA256" \
        --arg g21_shipper "$G21_SHIPPER_SHA256" \
        --arg g21_provisioner "$G21_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g21-g22-recovery-receipt.v1 \
        --arg previous_manifest "$G21_MANIFEST_SHA256" \
        --arg target_manifest "$G22_MANIFEST_SHA256" \
        --arg target_bundle "$G22_BUNDLE_SHA256" \
        --arg target_workflow "$G22_WORKFLOW_COMMIT" \
        --arg target_shipper "$G22_SHIPPER_SHA256" \
        --arg target_provisioner "$G22_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,completed:true,
          previous_generation:21,previous_manifest_sha256:$previous_manifest,
          target_generation:22,target_manifest_sha256:$target_manifest,
...
publish_generation_21() {
    local g22_dir=$1 destination=$ARTIFACT_ROOT/generation-22
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 22 "$g22_dir" "$G22_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g22_dir/release-authority-v2.json" \
        "$g22_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g22_dir/syntaur-build-authority-provision" \
        "$g22_dir/syntaur-ship-linux-x86_64" \
        "$g22_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G22_WORKFLOW_COMMIT" \
...
[[ -n $g21_dir && -n $g22_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g21_dir" "$g22_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G21-G22 signed successor verified: generation=22 manifest_sha256=%s\n' \
        "$G22_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G21_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G21'
...
if [[ $receipt_present == true \
    && $active_manifest == "$G22_MANIFEST_SHA256" ]] \
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G22_SHIPPER_SHA256" 1755 \
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G22_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'G22 authority status changed product or deployment state'
    assert_locks "$root_lock_identity" "$global_lock_identity" \
        "$deploy_lock_identity"
    if [[ $fence_present == true ]]; then
        preflight_recovery_temporaries
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G21-G22 authority recovery already installed: active_generation=22 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G22_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G22 is not complete'
preflight_recovery_temporaries
...
prepare_recovery_snapshot \
    "$g21_dir" "$g22_dir" "$operator_uid" "$operator_gid" "$snapshot_resume"
g21_material=$RECOVERY_SNAPSHOT/generation-21
g22_material=$RECOVERY_SNAPSHOT/generation-22
installed_executable_is_exact "$INSTALLED_PROVISIONER" \
    "$G22_PROVISIONER_SHA256" 755 \
    || die 'G21/G22 shared provisioner differs before recovery'
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G22_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G22_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G22 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G21_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G21 nor complete G22'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G21_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G21 nor complete G22'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G21_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G21'
    validate_installed_generation 21 "$g21_material" "$G21_WORKFLOW_COMMIT"
    validate_retained_chain 21
    validate_active_copy 21 "$g21_material" "$G21_WORKFLOW_COMMIT"
    run_operator_authority_status "$operator_uid" "$operator_gid"
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'G21 authority status changed product or deployment state'
    assert_locks "$root_lock_identity" "$global_lock_identity" \
        "$deploy_lock_identity"
    ...
    validate_current_journal_state "$transaction_product_digest"
    recovery_phase=$(/usr/bin/jq -er '.phase' "$RECOVERY_JOURNAL")
fi

if [[ $recovery_phase == prepared ]]; then
    assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
    publish_generation_21 "$g22_material"
    write_journal generation_published "$transaction_product_digest"
    validate_current_journal_state "$transaction_product_digest"
    recovery_phase=generation_published
fi

if [[ $recovery_phase == generation_published ]]; then
    publish_exact_file \
        "$ARTIFACT_ROOT/generation-22/syntaur-ship-linux-x86_64" \
        "$INSTALLED_SHIPPER" "$G22_SHIPPER_SHA256" 1755 \
        "$G21_SHIPPER_SHA256" 1755
    write_journal shipper_published "$transaction_product_digest"
    validate_current_journal_state "$transaction_product_digest"
    recovery_phase=shipper_published
fi

if [[ $recovery_phase == shipper_published ]]; then
    write_journal provisioner_published "$transaction_product_digest"
    validate_current_journal_state "$transaction_product_digest"
    recovery_phase=provisioner_published
fi

if [[ $recovery_phase == provisioner_published ]]; then
    g21_workflow_sha=$(workflow_file_sha256 "$G21_WORKFLOW_COMMIT")
    g22_workflow_sha=$(workflow_file_sha256 "$G22_WORKFLOW_COMMIT")
    publish_exact_file \
        "$ARTIFACT_ROOT/generation-22/trusted-workflow-commit" \
        "$AUTHORITY_ROOT/trusted-workflow-commit" \
        "$g22_workflow_sha" 444 "$g21_workflow_sha" 444
    write_journal trust_published "$transaction_product_digest"
    validate_current_journal_state "$transaction_product_digest"
    recovery_phase=trust_published
fi

if [[ $recovery_phase == trust_published ]]; then
    publish_exact_file \
        "$ARTIFACT_ROOT/generation-22/release-authority-v2.json.cosign.bundle" \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
        "$G22_BUNDLE_SHA256" 444 "$G21_BUNDLE_SHA256" 444
    write_journal bundle_published "$transaction_product_digest"
    validate_current_journal_state "$transaction_product_digest"
    recovery_phase=bundle_published
fi

if [[ $recovery_phase == bundle_published ]]; then
    assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
    publish_exact_file \
        "$ARTIFACT_ROOT/generation-22/release-authority-v2.json" \
        "$AUTHORITY_ROOT/release-authority-v2.json" \
        "$G22_MANIFEST_SHA256" 444 "$G21_MANIFEST_SHA256" 444
    write_journal manifest_published "$transaction_product_digest"
    validate_current_journal_state "$transaction_product_digest"
    recovery_phase=manifest_published
fi

[[ $recovery_phase == manifest_published ]] \
    || die 'recovery did not reach the exact terminal journal phase'

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
validate_installed_generation 21 "$g21_material" "$G21_WORKFLOW_COMMIT"
validate_installed_generation 22 "$g22_material" "$G22_WORKFLOW_COMMIT"
validate_retained_chain 22
validate_active_copy 22 "$g22_material" "$G22_WORKFLOW_COMMIT"
installed_executable_is_exact "$INSTALLED_SHIPPER" \
    "$G22_SHIPPER_SHA256" 1755 || die 'final G22 shipper differs'
installed_executable_is_exact "$INSTALLED_PROVISIONER" \
    "$G22_PROVISIONER_SHA256" 755 || die 'final G22 provisioner differs'
...
/usr/bin/printf 'G21-G22 authority recovery installed: active_generation=22 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G22_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
