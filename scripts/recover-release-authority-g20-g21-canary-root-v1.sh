...
readonly G20_MANIFEST_SHA256=c30b8dbd24a39fea8479aebe67432e76aef505702de9290b7e791fa2216e6e41
readonly G20_BUNDLE_SHA256=af33a84b0d6f3b0e4cadce541fa3a6751b8275921dc1de0204cc739d4320c4ab
readonly G20_WORKFLOW_COMMIT=6e4bdaf070c0d3d763531a2424fff93f2cb466a3
readonly G20_SHIPPER_SHA256=3e0180046c958e8491fe0f86e02592961af2d6ffb1d12643fcfc66a4238d8cb9
readonly G20_VERIFIER_SHA256=fbcf2efbd7a10c6b57e13e0f58bd419777034ee2835b027a73074b90ab83c9b2
readonly G20_PROVISIONER_SHA256=315bf5591b1ac00ffb443a8e8003fb77b7a421a5e66e91e6e9fd3ed5f859860b
...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g20-g21-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-21-g20-g21-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g20-g21-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g20-g21-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g20-g21-recovery-v1.runtime
...
die() {
    printf 'release authority G20-G21 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g20-g21-canary-root-v1.sh verify \
    --g20-dir DIR --g21-dir DIR

  sudo recover-release-authority-g20-g21-canary-root-v1.sh install \
    --g20-dir DIR --g21-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G20 authority root to
its exact immutable G21 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G21
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g20-g21-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g20-g21-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    local path
    for path in \
        "$AUTHORITY_ROOT/authority-g19-g20-recovery-v1.json" \
        "$AUTHORITY_ROOT/.authority-g19-g20-recovery-v1.json.tmp" \
        "$AUTHORITY_ROOT/.authority-g19-g20-recovery-v1.inputs" \
        "$AUTHORITY_ROOT/.authority-g19-g20-recovery-v1.inputs.staged" \
        "$AUTHORITY_ROOT/.authority-g19-g20-recovery-v1.inputs.retiring" \
        "$AUTHORITY_ROOT/.authority-g19-g20-recovery-v1.fence.tmp" \
        "$ARTIFACT_ROOT/.generation-20-g19-g20-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g19-g20-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G15-G20 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g20_dir=$1 g21_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G21_MANIFEST_SHA256" \
        || die 'G21 manifest digest was not fixed before publication'
    valid_sha256 "$G21_BUNDLE_SHA256" \
        || die 'G21 bundle digest was not fixed before publication'
    ...
    [[ $g20_dir != "$g21_dir" ]] || die 'G20 and G21 directories must differ'
    [[ $G20_SHIPPER_SHA256 != "$G21_SHIPPER_SHA256" ]] \
        || die 'G20 and G21 shipper digests must differ'
    [[ $G20_PROVISIONER_SHA256 == "$G21_PROVISIONER_SHA256" ]] \
        || die 'G20 and G21 must retain the exact shared provisioner'
    validate_material "$g20_dir" 20 \
        "$G20_MANIFEST_SHA256" "$G20_BUNDLE_SHA256" \
        "$G20_WORKFLOW_COMMIT" "$G20_SHIPPER_SHA256" \
        "$G20_VERIFIER_SHA256" "$G20_PROVISIONER_SHA256" \
    ...
    "$manifest_helper" assert-successor \
        "$g20_dir/release-authority-v2.json" \
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-20|generation-21|release-authority-manifest.sh) ;;
            *) die 'partial recovery input snapshot has an unexpected entry' ;;
        esac
    done <<<"$actual"
    for generation in 20 21; do
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-20" "$root/generation-21" 0 0 700 400 500
}

require_sources_match_snapshot() {
    local g20_dir=$1 g21_dir=$2 operator_uid=$3 operator_gid=$4
    local generation source name
    validate_material_layout "$g20_dir" 20 \
        "$operator_uid" "$operator_gid" 500 400 500
    validate_material_layout "$g21_dir" 21 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 20 21; do
        if [[ $generation == 20 ]]; then source=$g20_dir; else source=$g21_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g20_dir" "$g21_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
    ...
    /usr/bin/install -d -o root -g root -m 0700 \
        "$RECOVERY_SNAPSHOT_STAGE" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-20" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-21"
    snapshot_release "$g20_dir" "$RECOVERY_SNAPSHOT_STAGE/generation-20"
...
validate_complete_installed_g21() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_fixed_installed_generation 20 \
        "$G20_MANIFEST_SHA256" "$G20_BUNDLE_SHA256" \
        "$G20_WORKFLOW_COMMIT" "$G20_SHIPPER_SHA256" \
        "$G20_VERIFIER_SHA256" "$G20_PROVISIONER_SHA256"
    ...
    validate_active_copy 21 "$ARTIFACT_ROOT/generation-21" \
        "$G21_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G21_SHIPPER_SHA256" 1755 || die 'final G21 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G21_PROVISIONER_SHA256" 755 || die 'final G21 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g20_sha=$3 g21_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g20_sha" ]]; then
        /usr/bin/printf '20\n'
    elif [[ $digest == "$g21_sha" ]]; then
        /usr/bin/printf '21\n'
    else
...
validate_phase_state() {
    ...
    local manifest_state vector g20_workflow_sha g21_workflow_sha
    validate_installed_generation 20 \
        "$RECOVERY_SNAPSHOT/generation-20" "$G20_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-21 || -L $ARTIFACT_ROOT/generation-21 ]]; then
    ...
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G20_SHIPPER_SHA256" \
        "$G21_SHIPPER_SHA256" 'installed shipper')
    g20_workflow_sha=$(workflow_file_sha256 "$G20_WORKFLOW_COMMIT")
    ...
    trust_state=$(exact_generation_for_file \
        ...
        "$g20_workflow_sha" "$g21_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G20_BUNDLE_SHA256" "$G21_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G20_MANIFEST_SHA256" "$G21_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:20:20:20:20|prepared:21:20:20:20:20|\
        generation_published:21:20:20:20:20|\
        generation_published:21:21:20:20:20|\
        shipper_published:21:21:20:20:20|\
        provisioner_published:21:21:20:20:20|\
        trust_published:21:21:20:20:20|\
        trust_published:21:21:21:20:20|\
        bundle_published:21:21:21:20:20|\
        bundle_published:21:21:21:21:20|\
        manifest_published:21:21:21:21:21) ;;
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g20-g21-normal-mutation-fence.v1 \
        --arg recovery_journal authority-g20-g21-recovery-v1.json \
        --arg g20_manifest "$G20_MANIFEST_SHA256" \
        --arg g20_shipper "$G20_SHIPPER_SHA256" \
        --arg g20_provisioner "$G20_PROVISIONER_SHA256" \
        --arg g21_manifest "$G21_MANIFEST_SHA256" \
        --arg g21_bundle "$G21_BUNDLE_SHA256" \
        --arg g21_workflow "$G21_WORKFLOW_COMMIT" \
        ...
        '{schema:$schema,normal_mutations_blocked:true,
          ...
          previous_generation:20,previous_manifest_sha256:$g20_manifest,
          previous_shipper_sha256:$g20_shipper,
          previous_provisioner_sha256:$g20_provisioner,
          target_generation:21,target_manifest_sha256:$g21_manifest,
          target_bundle_sha256:$g21_bundle,
          target_workflow_commit:$g21_workflow,
          target_shipper_sha256:$g21_shipper,
          target_provisioner_sha256:$g21_provisioner,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g20-g21-recovery.v1 \
        --arg phase "$phase" \
        --arg g20_manifest "$G20_MANIFEST_SHA256" \
        --arg g20_shipper "$G20_SHIPPER_SHA256" \
        --arg g20_provisioner "$G20_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g20-g21-recovery-receipt.v1 \
        --arg previous_manifest "$G20_MANIFEST_SHA256" \
        --arg target_manifest "$G21_MANIFEST_SHA256" \
        --arg target_bundle "$G21_BUNDLE_SHA256" \
        --arg target_workflow "$G21_WORKFLOW_COMMIT" \
        --arg target_shipper "$G21_SHIPPER_SHA256" \
        --arg target_provisioner "$G21_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,completed:true,
          previous_generation:20,previous_manifest_sha256:$previous_manifest,
          target_generation:21,target_manifest_sha256:$target_manifest,
...
publish_generation_20() {
    local g21_dir=$1 destination=$ARTIFACT_ROOT/generation-21
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 21 "$g21_dir" "$G21_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g21_dir/release-authority-v2.json" \
        "$g21_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g21_dir/syntaur-build-authority-provision" \
        "$g21_dir/syntaur-ship-linux-x86_64" \
        "$g21_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G21_WORKFLOW_COMMIT" \
...
[[ -n $g20_dir && -n $g21_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g20_dir" "$g21_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G20-G21 signed successor verified: generation=21 manifest_sha256=%s\n' \
        "$G21_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G20_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G20'
...
if [[ $receipt_present == true \
    ...
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        ...
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G21_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G20-G21 authority recovery already installed: active_generation=21 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G21_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G21 is not complete'
preflight_recovery_temporaries
...
prepare_recovery_snapshot \
    "$g20_dir" "$g21_dir" "$operator_uid" "$operator_gid" "$snapshot_resume"
g20_material=$RECOVERY_SNAPSHOT/generation-20
g21_material=$RECOVERY_SNAPSHOT/generation-21
installed_executable_is_exact "$INSTALLED_PROVISIONER" \
    "$G21_PROVISIONER_SHA256" 755 \
    || die 'G20/G21 shared provisioner differs before recovery'
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G21_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G21_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G21 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G20_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G20 nor complete G21'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G20_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G20 nor complete G21'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G20_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G20'
    validate_installed_generation 20 "$g20_material" "$G20_WORKFLOW_COMMIT"
    validate_retained_chain 20
    validate_active_copy 20 "$g20_material" "$G20_WORKFLOW_COMMIT"
    run_operator_authority_status "$operator_uid" "$operator_gid"
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'G20 authority status changed product or deployment state'
    assert_locks "$root_lock_identity" "$global_lock_identity" \
        "$deploy_lock_identity"
...
assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_generation_20 "$g21_material"
write_journal generation_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-21/syntaur-ship-linux-x86_64" \
    "$INSTALLED_SHIPPER" "$G21_SHIPPER_SHA256" 1755 \
    "$G20_SHIPPER_SHA256" 1755
...
g21_workflow_sha=$(workflow_file_sha256 "$G21_WORKFLOW_COMMIT")
publish_exact_file \
    "$ARTIFACT_ROOT/generation-21/trusted-workflow-commit" \
    "$AUTHORITY_ROOT/trusted-workflow-commit" \
    "$g21_workflow_sha" 444 "$g20_workflow_sha" 444
write_journal trust_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-21/release-authority-v2.json.cosign.bundle" \
    "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
    "$G21_BUNDLE_SHA256" 444 "$G20_BUNDLE_SHA256" 444
write_journal bundle_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_exact_file \
    "$ARTIFACT_ROOT/generation-21/release-authority-v2.json" \
    "$AUTHORITY_ROOT/release-authority-v2.json" \
    "$G21_MANIFEST_SHA256" 444 "$G20_MANIFEST_SHA256" 444
...
/usr/bin/printf 'G20-G21 authority recovery installed: active_generation=21 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G21_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
