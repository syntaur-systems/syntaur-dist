...
readonly G13_MANIFEST_SHA256=914bba73bfa835d4906ae7ff6f8d743220e79b2b21a8ebe73fd6f2d3477507de
readonly G13_BUNDLE_SHA256=3bb52fb9a01ed52fbedf925159f04f62b4d434f2989ea5b9ae56209ac0555ad4
readonly G13_WORKFLOW_COMMIT=07e6728559f4fce53aaacce986946e467e632942
readonly G13_SHIPPER_SHA256=68f2017a1d5303b4cf3ee238b1b372c92ac0e3fc11fe284553e8845ce1b480ce
readonly G13_VERIFIER_SHA256=4b050fe880b8dbd4a6d41b24e305837a117c9be7b7a21c6b6e55552f5d6f35a5
readonly G13_PROVISIONER_SHA256=315bf5591b1ac00ffb443a8e8003fb77b7a421a5e66e91e6e9fd3ed5f859860b

readonly G14_MANIFEST_SHA256=ee9052a8eb2d3688ef9a5447ee6ac16ca267500009a2aa88be2c680e9799a9f8
readonly G14_BUNDLE_SHA256=592e43039950b8ed9dc1180a14dde6eaedc5a5381f8d095bf5464c5d52b5156b
readonly G14_WORKFLOW_COMMIT=608f3e0f1dd7076fdad2284173928a4453ef9090
readonly G14_SHIPPER_SHA256=591e37ae154078974126ca1640991494c8d211cd2fc403a941dfa59193a94f2a
readonly G14_VERIFIER_SHA256=31a438b3e3b9a8a11347bcf5db5460ef1adf92a0196520c1252311f4541d9bdd
readonly G14_PROVISIONER_SHA256=315bf5591b1ac00ffb443a8e8003fb77b7a421a5e66e91e6e9fd3ed5f859860b
...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g13-g14-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-14-g13-g14-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g13-g14-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g13-g14-recovery-v1.runtime
...
die() {
    printf 'release authority G13-G14 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g13-g14-canary-root-v1.sh verify \
    --g13-dir DIR --g14-dir DIR

  sudo recover-release-authority-g13-g14-canary-root-v1.sh install \
    --g13-dir DIR --g14-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G13 authority root to
its exact immutable G14 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G14
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g13-g14-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g13-g14-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    local path
    for path in \
        "$AUTHORITY_ROOT/authority-g12-g13-recovery-v1.json" \
        "$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.json.tmp" \
        "$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.inputs" \
        "$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.inputs.staged" \
        "$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.inputs.retiring" \
        "$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.fence.tmp" \
        "$ARTIFACT_ROOT/.generation-13-g12-g13-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g12-g13-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G12-G13 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g13_dir=$1 g14_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G14_MANIFEST_SHA256" \
        || die 'G14 manifest digest was not fixed before publication'
    valid_sha256 "$G14_BUNDLE_SHA256" \
        || die 'G14 bundle digest was not fixed before publication'
    ...
    [[ $g13_dir != "$g14_dir" ]] || die 'G13 and G14 directories must differ'
    [[ $G13_SHIPPER_SHA256 != "$G14_SHIPPER_SHA256" ]] \
        || die 'G13 and G14 shipper digests must differ'
    [[ $G13_PROVISIONER_SHA256 == "$G14_PROVISIONER_SHA256" ]] \
        || die 'G13 and G14 must retain the exact shared provisioner'
    validate_material "$g13_dir" 13 \
        "$G13_MANIFEST_SHA256" "$G13_BUNDLE_SHA256" \
        "$G13_WORKFLOW_COMMIT" "$G13_SHIPPER_SHA256" \
        "$G13_VERIFIER_SHA256" "$G13_PROVISIONER_SHA256" \
        "$operator_uid" "$operator_gid" "$directory_mode" \
        "$data_mode" "$executable_mode"
    validate_material "$g14_dir" 14 \
        "$G14_MANIFEST_SHA256" "$G14_BUNDLE_SHA256" \
        "$G14_WORKFLOW_COMMIT" "$G14_SHIPPER_SHA256" \
        "$G14_VERIFIER_SHA256" "$G14_PROVISIONER_SHA256" \
        "$operator_uid" "$operator_gid" "$directory_mode" \
        "$data_mode" "$executable_mode"
    "$manifest_helper" assert-successor \
        "$g13_dir/release-authority-v2.json" \
        "$g14_dir/release-authority-v2.json"
}
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-13|generation-14|release-authority-manifest.sh) ;;
            *) die 'partial recovery input snapshot has an unexpected entry' ;;
        esac
    done <<<"$actual"
    for generation in 13 14; do
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-13" "$root/generation-14" 0 0 700 400 500
}

require_sources_match_snapshot() {
    local g13_dir=$1 g14_dir=$2 operator_uid=$3 operator_gid=$4
    local generation source name
    validate_material_layout "$g13_dir" 13 \
        "$operator_uid" "$operator_gid" 500 400 500
    validate_material_layout "$g14_dir" 14 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 13 14; do
        if [[ $generation == 13 ]]; then source=$g13_dir; else source=$g14_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g13_dir" "$g14_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
    ...
    /usr/bin/install -d -o root -g root -m 0700 \
        "$RECOVERY_SNAPSHOT_STAGE" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-13" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-14"
    snapshot_release "$g13_dir" "$RECOVERY_SNAPSHOT_STAGE/generation-13"
    snapshot_release "$g14_dir" "$RECOVERY_SNAPSHOT_STAGE/generation-14"
...
validate_complete_installed_g14() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_fixed_installed_generation 13 \
        "$G13_MANIFEST_SHA256" "$G13_BUNDLE_SHA256" \
        "$G13_WORKFLOW_COMMIT" "$G13_SHIPPER_SHA256" \
        "$G13_VERIFIER_SHA256" "$G13_PROVISIONER_SHA256"
    validate_fixed_installed_generation 14 \
        "$G14_MANIFEST_SHA256" "$G14_BUNDLE_SHA256" \
        "$G14_WORKFLOW_COMMIT" "$G14_SHIPPER_SHA256" \
        "$G14_VERIFIER_SHA256" "$G14_PROVISIONER_SHA256"
    validate_retained_chain 14
    validate_active_copy 14 "$ARTIFACT_ROOT/generation-14" \
        "$G14_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G14_SHIPPER_SHA256" 1755 || die 'final G14 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G14_PROVISIONER_SHA256" 755 || die 'final G14 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g13_sha=$3 g14_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g13_sha" ]]; then
        /usr/bin/printf '13\n'
    elif [[ $digest == "$g14_sha" ]]; then
        /usr/bin/printf '14\n'
    else
...
validate_phase_state() {
    ...
    local manifest_state vector g13_workflow_sha g14_workflow_sha
    validate_installed_generation 13 \
        "$RECOVERY_SNAPSHOT/generation-13" "$G13_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-14 || -L $ARTIFACT_ROOT/generation-14 ]]; then
        validate_installed_generation 14 \
            "$RECOVERY_SNAPSHOT/generation-14" "$G14_WORKFLOW_COMMIT"
        generation_state=14
    else
        generation_state=0
    fi
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G13_SHIPPER_SHA256" \
        "$G14_SHIPPER_SHA256" 'installed shipper')
    g13_workflow_sha=$(workflow_file_sha256 "$G13_WORKFLOW_COMMIT")
    g14_workflow_sha=$(workflow_file_sha256 "$G14_WORKFLOW_COMMIT")
    trust_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/trusted-workflow-commit" 444 \
        "$g13_workflow_sha" "$g14_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G13_BUNDLE_SHA256" "$G14_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G13_MANIFEST_SHA256" "$G14_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:13:13:13:13|prepared:14:13:13:13:13|\
        generation_published:14:13:13:13:13|\
        generation_published:14:14:13:13:13|\
        shipper_published:14:14:13:13:13|\
        provisioner_published:14:14:13:13:13|\
        provisioner_published:14:14:14:13:13|\
        trust_published:14:14:14:13:13|\
        trust_published:14:14:14:14:13|\
        bundle_published:14:14:14:14:13|\
        bundle_published:14:14:14:14:14|\
        manifest_published:14:14:14:14:14) ;;
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g13-g14-normal-mutation-fence.v1 \
        --arg recovery_journal authority-g13-g14-recovery-v1.json \
        --arg g13_manifest "$G13_MANIFEST_SHA256" \
        --arg g13_shipper "$G13_SHIPPER_SHA256" \
        --arg g13_provisioner "$G13_PROVISIONER_SHA256" \
        --arg g14_manifest "$G14_MANIFEST_SHA256" \
        --arg g14_bundle "$G14_BUNDLE_SHA256" \
        --arg g14_workflow "$G14_WORKFLOW_COMMIT" \
        --arg g14_shipper "$G14_SHIPPER_SHA256" \
        --arg g14_provisioner "$G14_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,normal_mutations_blocked:true,
          recovery_journal:$recovery_journal,
          previous_generation:13,previous_manifest_sha256:$g13_manifest,
          previous_shipper_sha256:$g13_shipper,
          previous_provisioner_sha256:$g13_provisioner,
          target_generation:14,target_manifest_sha256:$g14_manifest,
          target_bundle_sha256:$g14_bundle,
          target_workflow_commit:$g14_workflow,
          target_shipper_sha256:$g14_shipper,
          target_provisioner_sha256:$g14_provisioner,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g13-g14-recovery.v1 \
        --arg phase "$phase" \
        --arg g13_manifest "$G13_MANIFEST_SHA256" \
        --arg g13_shipper "$G13_SHIPPER_SHA256" \
        --arg g13_provisioner "$G13_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g13-g14-recovery-receipt.v1 \
        --arg previous_manifest "$G13_MANIFEST_SHA256" \
        --arg target_manifest "$G14_MANIFEST_SHA256" \
        --arg target_bundle "$G14_BUNDLE_SHA256" \
        --arg target_workflow "$G14_WORKFLOW_COMMIT" \
        --arg target_shipper "$G14_SHIPPER_SHA256" \
        --arg target_provisioner "$G14_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,completed:true,
          previous_generation:13,previous_manifest_sha256:$previous_manifest,
          target_generation:14,target_manifest_sha256:$target_manifest,
...
publish_generation_14() {
    local g14_dir=$1 destination=$ARTIFACT_ROOT/generation-14
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 14 "$g14_dir" "$G14_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g14_dir/release-authority-v2.json" \
        "$g14_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g14_dir/syntaur-build-authority-provision" \
        "$g14_dir/syntaur-ship-linux-x86_64" \
        "$g14_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G14_WORKFLOW_COMMIT" \
...
[[ -n $g13_dir && -n $g14_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g13_dir" "$g14_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G13-G14 signed successor verified: generation=14 manifest_sha256=%s\n' \
        "$G14_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G13_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G13'
...
if [[ $receipt_present == true \
    && $active_manifest == "$G14_MANIFEST_SHA256" ]] \
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G14_SHIPPER_SHA256" 1755 \
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G14_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'G14 authority status changed product or deployment state'
    assert_locks "$root_lock_identity" "$global_lock_identity" \
        "$deploy_lock_identity"
    if [[ $fence_present == true ]]; then
        preflight_recovery_temporaries
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G13-G14 authority recovery already installed: active_generation=14 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G14_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G14 is not complete'
preflight_recovery_temporaries
...
prepare_recovery_snapshot \
    "$g13_dir" "$g14_dir" "$operator_uid" "$operator_gid" "$snapshot_resume"
g13_material=$RECOVERY_SNAPSHOT/generation-13
g14_material=$RECOVERY_SNAPSHOT/generation-14
installed_executable_is_exact "$INSTALLED_PROVISIONER" \
    "$G14_PROVISIONER_SHA256" 755 \
    || die 'G13/G14 shared provisioner differs before recovery'
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G14_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G14_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G14 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G13_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G13 nor complete G14'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G13_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G13 nor complete G14'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G13_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G13'
    validate_installed_generation 13 "$g13_material" "$G13_WORKFLOW_COMMIT"
    validate_retained_chain 13
    validate_active_copy 13 "$g13_material" "$G13_WORKFLOW_COMMIT"
    run_operator_authority_status "$operator_uid" "$operator_gid"
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'G13 authority status changed product or deployment state'
    assert_locks "$root_lock_identity" "$global_lock_identity" \
        "$deploy_lock_identity"
...
assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_generation_14 "$g14_material"
write_journal generation_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-14/syntaur-ship-linux-x86_64" \
    "$INSTALLED_SHIPPER" "$G14_SHIPPER_SHA256" 1755 \
    "$G13_SHIPPER_SHA256" 1755
...
g14_workflow_sha=$(workflow_file_sha256 "$G14_WORKFLOW_COMMIT")
publish_exact_file \
    "$ARTIFACT_ROOT/generation-14/trusted-workflow-commit" \
    "$AUTHORITY_ROOT/trusted-workflow-commit" \
    "$g14_workflow_sha" 444 "$g13_workflow_sha" 444
write_journal trust_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-14/release-authority-v2.json.cosign.bundle" \
    "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
    "$G14_BUNDLE_SHA256" 444 "$G13_BUNDLE_SHA256" 444
write_journal bundle_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_exact_file \
    "$ARTIFACT_ROOT/generation-14/release-authority-v2.json" \
    "$AUTHORITY_ROOT/release-authority-v2.json" \
    "$G14_MANIFEST_SHA256" 444 "$G13_MANIFEST_SHA256" 444
write_journal manifest_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
validate_installed_generation 13 "$g13_material" "$G13_WORKFLOW_COMMIT"
validate_installed_generation 14 "$g14_material" "$G14_WORKFLOW_COMMIT"
validate_retained_chain 14
validate_active_copy 14 "$g14_material" "$G14_WORKFLOW_COMMIT"
installed_executable_is_exact "$INSTALLED_SHIPPER" \
    "$G14_SHIPPER_SHA256" 1755 || die 'final G14 shipper differs'
installed_executable_is_exact "$INSTALLED_PROVISIONER" \
    "$G14_PROVISIONER_SHA256" 755 || die 'final G14 provisioner differs'
...
/usr/bin/printf 'G13-G14 authority recovery installed: active_generation=14 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G14_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
