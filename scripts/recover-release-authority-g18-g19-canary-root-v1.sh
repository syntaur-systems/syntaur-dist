...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g18-g19-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g18-g19-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-19-g18-g19-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g18-g19-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g18-g19-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g18-g19-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g18-g19-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g18-g19-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g18-g19-recovery-v1.runtime
...
die() {
    printf 'release authority G18-G19 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g18-g19-canary-root-v1.sh verify \
    --g18-dir DIR --g19-dir DIR

  sudo recover-release-authority-g18-g19-canary-root-v1.sh install \
    --g18-dir DIR --g19-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G18 authority root to
its exact immutable G19 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G19
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g18-g19-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g18-g19-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    local path
    for path in \
        "$AUTHORITY_ROOT/authority-g17-g18-recovery-v1.json" \
        "$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.json.tmp" \
        "$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.inputs" \
        "$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.inputs.staged" \
        "$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.inputs.retiring" \
        "$AUTHORITY_ROOT/.authority-g17-g18-recovery-v1.fence.tmp" \
        "$ARTIFACT_ROOT/.generation-18-g17-g18-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g17-g18-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G15-G18 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g18_dir=$1 g19_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G19_MANIFEST_SHA256" \
        || die 'G19 manifest digest was not fixed before publication'
    valid_sha256 "$G19_BUNDLE_SHA256" \
        || die 'G19 bundle digest was not fixed before publication'
    ...
    [[ $g18_dir != "$g19_dir" ]] || die 'G18 and G19 directories must differ'
    [[ $G18_SHIPPER_SHA256 != "$G19_SHIPPER_SHA256" ]] \
        || die 'G18 and G19 shipper digests must differ'
    [[ $G18_PROVISIONER_SHA256 == "$G19_PROVISIONER_SHA256" ]] \
        || die 'G18 and G19 must retain the exact shared provisioner'
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-18|generation-19|release-authority-manifest.sh) ;;
            *) die 'partial recovery input snapshot has an unexpected entry' ;;
        esac
    done <<<"$actual"
    for generation in 18 19; do
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-18" "$root/generation-19" 0 0 700 400 500
}
...
require_sources_match_snapshot() {
    ...
    validate_material_layout "$g19_dir" 19 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 18 19; do
        if [[ $generation == 18 ]]; then source=$g18_dir; else source=$g19_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g18_dir" "$g19_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
...
validate_complete_installed_g19() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_active_copy 19 "$ARTIFACT_ROOT/generation-19" \
        "$G19_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G19_SHIPPER_SHA256" 1755 || die 'final G19 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G19_PROVISIONER_SHA256" 755 || die 'final G19 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g18_sha=$3 g19_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g18_sha" ]]; then
        /usr/bin/printf '18\n'
    elif [[ $digest == "$g19_sha" ]]; then
        /usr/bin/printf '19\n'
    else
...
validate_phase_state() {
    ...
    local manifest_state vector g18_workflow_sha g19_workflow_sha
    validate_installed_generation 18 \
        "$RECOVERY_SNAPSHOT/generation-18" "$G18_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-19 || -L $ARTIFACT_ROOT/generation-19 ]]; then
    ...
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G18_SHIPPER_SHA256" \
        "$G19_SHIPPER_SHA256" 'installed shipper')
    ...
    trust_state=$(exact_generation_for_file \
        ...
        "$g18_workflow_sha" "$g19_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G18_BUNDLE_SHA256" "$G19_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G18_MANIFEST_SHA256" "$G19_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:18:18:18:18|prepared:19:18:18:18:18|\
        generation_published:19:18:18:18:18|\
        generation_published:19:19:18:18:18|\
        shipper_published:19:19:18:18:18|\
        provisioner_published:19:19:18:18:18|\
        provisioner_published:19:19:19:18:18|\
        trust_published:19:19:19:18:18|\
        trust_published:19:19:19:19:18|\
        bundle_published:19:19:19:19:18|\
        bundle_published:19:19:19:19:19|\
        manifest_published:19:19:19:19:19) ;;
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g18-g19-normal-mutation-fence.v1 \
        --arg recovery_journal authority-g18-g19-recovery-v1.json \
        ...
        --arg g19_bundle "$G19_BUNDLE_SHA256" \
        --arg g19_workflow "$G19_WORKFLOW_COMMIT" \
        ...
        '{schema:$schema,normal_mutations_blocked:true,
          ...
          previous_generation:18,previous_manifest_sha256:$g18_manifest,
          previous_shipper_sha256:$g18_shipper,
          previous_provisioner_sha256:$g18_provisioner,
          target_generation:19,target_manifest_sha256:$g19_manifest,
          target_bundle_sha256:$g19_bundle,
          target_workflow_commit:$g19_workflow,
          target_shipper_sha256:$g19_shipper,
          target_provisioner_sha256:$g19_provisioner,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g18-g19-recovery.v1 \
        --arg phase "$phase" \
        --arg g18_manifest "$G18_MANIFEST_SHA256" \
        --arg g18_shipper "$G18_SHIPPER_SHA256" \
        --arg g18_provisioner "$G18_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g18-g19-recovery-receipt.v1 \
        --arg previous_manifest "$G18_MANIFEST_SHA256" \
        --arg target_manifest "$G19_MANIFEST_SHA256" \
        --arg target_bundle "$G19_BUNDLE_SHA256" \
        --arg target_workflow "$G19_WORKFLOW_COMMIT" \
        --arg target_shipper "$G19_SHIPPER_SHA256" \
        --arg target_provisioner "$G19_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,completed:true,
          previous_generation:18,previous_manifest_sha256:$previous_manifest,
          target_generation:19,target_manifest_sha256:$target_manifest,
...
publish_generation_18() {
    local g19_dir=$1 destination=$ARTIFACT_ROOT/generation-19
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 19 "$g19_dir" "$G19_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g19_dir/release-authority-v2.json" \
        "$g19_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g19_dir/syntaur-build-authority-provision" \
        "$g19_dir/syntaur-ship-linux-x86_64" \
        "$g19_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G19_WORKFLOW_COMMIT" \
...
[[ -n $g18_dir && -n $g19_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g18_dir" "$g19_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G18-G19 signed successor verified: generation=19 manifest_sha256=%s\n' \
        "$G19_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G18_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G18'
...
if [[ $receipt_present == true \
    ...
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        ...
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G19_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G18-G19 authority recovery already installed: active_generation=19 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G19_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G19 is not complete'
preflight_recovery_temporaries
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G19_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G19_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G19 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G18_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G18 nor complete G19'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G18_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G18 nor complete G19'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G18_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G18'
...
assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_generation_18 "$g19_material"
write_journal generation_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-19/syntaur-ship-linux-x86_64" \
    "$INSTALLED_SHIPPER" "$G19_SHIPPER_SHA256" 1755 \
    "$G18_SHIPPER_SHA256" 1755
...
g19_workflow_sha=$(workflow_file_sha256 "$G19_WORKFLOW_COMMIT")
publish_exact_file \
    "$ARTIFACT_ROOT/generation-19/trusted-workflow-commit" \
    "$AUTHORITY_ROOT/trusted-workflow-commit" \
    "$g19_workflow_sha" 444 "$g18_workflow_sha" 444
write_journal trust_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-19/release-authority-v2.json.cosign.bundle" \
    "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
    "$G19_BUNDLE_SHA256" 444 "$G18_BUNDLE_SHA256" 444
write_journal bundle_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_exact_file \
    "$ARTIFACT_ROOT/generation-19/release-authority-v2.json" \
    "$AUTHORITY_ROOT/release-authority-v2.json" \
    "$G19_MANIFEST_SHA256" 444 "$G18_MANIFEST_SHA256" 444
...
/usr/bin/printf 'G18-G19 authority recovery installed: active_generation=19 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G19_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
