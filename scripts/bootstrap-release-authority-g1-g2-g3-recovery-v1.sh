...
readonly G2_AUTHORITY_GIT_TREE=a1981a0868ba4d9d72128ca0c0d4ee82d466de2d
readonly G2_AUTHORITY_TREE_SHA256=b48fe9158b51ab69f49b233c6913fd2ec241b6d1446a3c455be15eb3932c1a63
readonly G2_SOURCE_DATE_EPOCH=1785438256
...
die() {
    printf 'release authority G1-G2-G3 recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  bootstrap-release-authority-g1-g2-g3-recovery-v1.sh verify|stage-g2-build-authority|install \
    --g1-dir DIR --g2-dir DIR --g3-dir DIR \
...
G1, G2, and G3. `verify` is non-mutating. `stage-g2-build-authority`
CAS-installs the exact G2 provisioner and G2 Genesis validator while the
release root remains absent. After external G2 Genesis validation, `install`
atomically publishes the retained G1-G2-G3 chain with G3 active.
EOF
    exit 2
}
...
action=$1
shift
[[ $action == verify || $action == stage-g2-build-authority \
...
for source_dir in "$g1_dir" "$g2_dir" "$g3_dir"; do
...
[[ $g1_dir != "$g2_dir" && $g1_dir != "$g3_dir" && $g2_dir != "$g3_dir" ]] \
...
validate_release_chain \
    ...
    "$g1_dir" "$g2_dir" "$g3_dir"

if [[ $action == verify ]]; then
    printf 'G1-G2-G3 recovery chain verified: active_generation=3 manifest_sha256=%s\n' \
        "$G3_MANIFEST_SHA256"
    exit 0
fi
...
snapshot=/run/syntaur-release-authority-g1-g2-g3-recovery.snapshot
...
root_stage=/etc/syntaur/.release-authority.recovery-v1-g1-g2-g3
shipper_stage=/usr/local/bin/.syntaur-ship.recovery-v1-g1-g2-g3
provisioner_stage=/opt/.syntaur-build-authority-provision.recovery-v1-g2
validator_stage=/opt/.syntaur-genesis-validator.recovery-v1-g2
...
if [[ $action == stage-g2-build-authority ]]; then
    provisioner_state=
    validator_state=
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G2 staging requires the release root to remain absent'
    if installed_executable_is_exact \
        ...
        "$INSTALLED_PROVISIONER" "$G1_PROVISIONER_SHA256"; then
        provisioner_state=g1
    else
        die 'installed provisioner is neither the exact G1 predecessor nor G2'
    fi
    if installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G2_SHIPPER_SHA256"; then
        validator_state=g2
    elif installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G1_SHIPPER_SHA256"; then
        validator_state=g1
    else
        die 'Genesis validator is neither the exact G1 predecessor nor G2'
    fi
    if [[ $provisioner_state == g1 ]]; then
        install_exact_executable \
            "$snapshot/generation-2/syntaur-build-authority-provision" \
            "$provisioner_stage" "$INSTALLED_PROVISIONER" \
            "$G2_PROVISIONER_SHA256"
    fi
    if [[ $validator_state == g1 ]]; then
        install_exact_executable \
            "$snapshot/generation-2/syntaur-ship-linux-x86_64" \
            "$validator_stage" "$GENESIS_VALIDATOR" \
            "$G2_SHIPPER_SHA256"
    fi
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G2 staging published the release root'
    printf 'G2 Genesis tools staged: provisioner_sha256=%s validator_sha256=%s release_root=absent\n' \
        "$G2_PROVISIONER_SHA256" "$G2_SHIPPER_SHA256"
    exit 0
fi
...
validate_genesis_evidence() {
    ...
    jq -se \
        ...
        --arg source "$G2_AUTHORITY_COMMIT" \
        --arg source_tree "$G2_AUTHORITY_GIT_TREE" \
        --arg engine "$ENGINE_COMMIT" \
        --arg engine_tree "$ENGINE_TREE" \
        --arg shipper "$G2_SHIPPER_SHA256" \
        --arg provisioner "$G2_PROVISIONER_SHA256" \
        ...
        --argjson source_epoch "$G2_SOURCE_DATE_EPOCH" \
        --argjson shipper_size "$shipper_size" \
        --slurpfile manifest \
            "$snapshot/generation-2/release-authority-v2.json" '
        ...
        ' "$evidence" >/dev/null \
        || die 'Genesis evidence does not satisfy the exact G2 contract'
}
...
validate_persistent_build_authority() {
    ...
    catalog="$root/catalog/$G2_AUTHORITY_COMMIT-$ENGINE_COMMIT.json"
    [[ -f $catalog && ! -L $catalog ]] \
        || die 'exact G2 persistent catalog is missing'
    [[ $(stat -c '%u:%g:%a:%h' "$catalog") == \
        "0:$operator_gid:440:1" ]] \
        || die 'exact G2 persistent catalog identity differs'
    [[ $(stat -c '%s' "$catalog") -gt 0 \
        && $(stat -c '%s' "$catalog") -le 65536 ]] \
        || die 'exact G2 persistent catalog size is invalid'
    [[ $(wc -l <"$catalog") -eq 1 ]] \
...
genesis_receipt_json() {
    jq -cn \
        --arg authority_commit "$G2_AUTHORITY_COMMIT" \
        --arg manifest_sha256 "$G2_MANIFEST_SHA256" \
        --arg evidence_sha256 "$expected_genesis_evidence_sha256" \
        --arg engine_commit "$ENGINE_COMMIT" \
        --arg rustsec_commit "$RUSTSEC_DB_COMMIT" \
        --arg shipper_sha256 "$G2_SHIPPER_SHA256" \
        --arg provisioner_sha256 "$G2_PROVISIONER_SHA256" \
        '{
...
validate_genesis_receipt() {
    ...
    [[ $canonical == "$expected" ]] \
        || die 'Genesis receipt differs from the G2 independent record'
}
...
validate_installed_layout() {
    ...
    for name in release-authority-v2.json \
        ...
        /usr/bin/cmp -s "$AUTHORITY_ROOT/$name" \
            "$AUTHORITY_ROOT/release-authority/generation-3/$name" \
...
preflight_live_shipper() {
    ...
    if [[ $authority_root_exists == true ]] \
        && installed_executable_is_exact \
            "$INSTALLED_SHIPPER" "$G3_SHIPPER_SHA256" 1755; then
        return
    fi
    installed_executable_is_exact \
...
remove_exact_genesis_validator() {
    ...
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G2_SHIPPER_SHA256" \
        || die 'G2 Genesis validator changed before retirement'
...
validate_mac_material
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G2_PROVISIONER_SHA256" \
    || die 'install requires the exact staged G2/G3 provisioner'
if [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]]; then
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G2_SHIPPER_SHA256" \
        || die 'install requires the exact staged G2 Genesis validator'
fi
...
if [[ $authority_root_exists == false ]]; then
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$snapshot/generation-3/release-authority-v2.json" \
        "$root_stage/release-authority-v2.json"
    /usr/bin/install -o root -g root -m 0444 \
        "$snapshot/generation-3/release-authority-v2.json.cosign.bundle" \
...
if ! installed_executable_is_exact \
    "$INSTALLED_SHIPPER" "$G3_SHIPPER_SHA256" 1755; then
    install_exact_executable \
        "$snapshot/generation-3/syntaur-ship-linux-x86_64" \
        "$shipper_stage" "$INSTALLED_SHIPPER" "$G3_SHIPPER_SHA256" 1755
fi
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G3_PROVISIONER_SHA256" \
    || die 'active G3 provisioner differs'
run_operator_authority_status
remove_exact_genesis_validator
printf 'G1-G2-G3 authority recovery installed: active_generation=3 manifest_sha256=%s\n' \
    "$G3_MANIFEST_SHA256"
