...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g14-g15-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g14-g15-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-15-g14-g15-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g14-g15-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g14-g15-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g14-g15-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g14-g15-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g14-g15-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g14-g15-recovery-v1.runtime
...
die() {
    printf 'release authority G14-G15 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g14-g15-canary-root-v1.sh verify \
    --g14-dir DIR --g15-dir DIR

  sudo recover-release-authority-g14-g15-canary-root-v1.sh install \
    --g14-dir DIR --g15-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G14 authority root to
its exact immutable G15 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G15
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g14-g15-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g14-g15-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    local path
    for path in \
        "$AUTHORITY_ROOT/authority-g13-g14-recovery-v1.json" \
        "$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.json.tmp" \
        "$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.inputs" \
        "$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.inputs.staged" \
        "$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.inputs.retiring" \
        "$AUTHORITY_ROOT/.authority-g13-g14-recovery-v1.fence.tmp" \
        "$ARTIFACT_ROOT/.generation-14-g13-g14-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g13-g14-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G13-G14 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g14_dir=$1 g15_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G15_MANIFEST_SHA256" \
        || die 'G15 manifest digest was not fixed before publication'
    valid_sha256 "$G15_BUNDLE_SHA256" \
        || die 'G15 bundle digest was not fixed before publication'
    ...
    [[ $g14_dir != "$g15_dir" ]] || die 'G14 and G15 directories must differ'
    [[ $G14_SHIPPER_SHA256 != "$G15_SHIPPER_SHA256" ]] \
        || die 'G14 and G15 shipper digests must differ'
    [[ $G14_PROVISIONER_SHA256 == "$G15_PROVISIONER_SHA256" ]] \
        || die 'G14 and G15 must retain the exact shared provisioner'
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-14|generation-15|release-authority-manifest.sh) ;;
            *) die 'partial recovery input snapshot has an unexpected entry' ;;
        esac
    done <<<"$actual"
    for generation in 14 15; do
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-14" "$root/generation-15" 0 0 700 400 500
}
...
require_sources_match_snapshot() {
    ...
    validate_material_layout "$g15_dir" 15 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 14 15; do
        if [[ $generation == 14 ]]; then source=$g14_dir; else source=$g15_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g14_dir" "$g15_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
...
validate_complete_installed_g15() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_active_copy 15 "$ARTIFACT_ROOT/generation-15" \
        "$G15_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G15_SHIPPER_SHA256" 1755 || die 'final G15 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G15_PROVISIONER_SHA256" 755 || die 'final G15 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g14_sha=$3 g15_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g14_sha" ]]; then
        /usr/bin/printf '14\n'
    elif [[ $digest == "$g15_sha" ]]; then
        /usr/bin/printf '15\n'
    else
...
validate_phase_state() {
    ...
    local manifest_state vector g14_workflow_sha g15_workflow_sha
    validate_installed_generation 14 \
        "$RECOVERY_SNAPSHOT/generation-14" "$G14_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-15 || -L $ARTIFACT_ROOT/generation-15 ]]; then
    ...
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G14_SHIPPER_SHA256" \
        "$G15_SHIPPER_SHA256" 'installed shipper')
    ...
    trust_state=$(exact_generation_for_file \
        ...
        "$g14_workflow_sha" "$g15_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G14_BUNDLE_SHA256" "$G15_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G14_MANIFEST_SHA256" "$G15_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:14:14:14:14|prepared:15:14:14:14:14|\
        generation_published:15:14:14:14:14|\
        generation_published:15:15:14:14:14|\
        shipper_published:15:15:14:14:14|\
        provisioner_published:15:15:14:14:14|\
        provisioner_published:15:15:15:14:14|\
        trust_published:15:15:15:14:14|\
        trust_published:15:15:15:15:14|\
        bundle_published:15:15:15:15:14|\
        bundle_published:15:15:15:15:15|\
        manifest_published:15:15:15:15:15) ;;
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g14-g15-normal-mutation-fence.v1 \
        --arg recovery_journal authority-g14-g15-recovery-v1.json \
        ...
        --arg g15_bundle "$G15_BUNDLE_SHA256" \
        --arg g15_workflow "$G15_WORKFLOW_COMMIT" \
        ...
        '{schema:$schema,normal_mutations_blocked:true,
          ...
          previous_generation:14,previous_manifest_sha256:$g14_manifest,
          previous_shipper_sha256:$g14_shipper,
          previous_provisioner_sha256:$g14_provisioner,
          target_generation:15,target_manifest_sha256:$g15_manifest,
          target_bundle_sha256:$g15_bundle,
          target_workflow_commit:$g15_workflow,
          target_shipper_sha256:$g15_shipper,
          target_provisioner_sha256:$g15_provisioner,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g14-g15-recovery.v1 \
        --arg phase "$phase" \
        --arg g14_manifest "$G14_MANIFEST_SHA256" \
        --arg g14_shipper "$G14_SHIPPER_SHA256" \
        --arg g14_provisioner "$G14_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g14-g15-recovery-receipt.v1 \
        --arg previous_manifest "$G14_MANIFEST_SHA256" \
        --arg target_manifest "$G15_MANIFEST_SHA256" \
        --arg target_bundle "$G15_BUNDLE_SHA256" \
        --arg target_workflow "$G15_WORKFLOW_COMMIT" \
        --arg target_shipper "$G15_SHIPPER_SHA256" \
        --arg target_provisioner "$G15_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,completed:true,
          previous_generation:14,previous_manifest_sha256:$previous_manifest,
          target_generation:15,target_manifest_sha256:$target_manifest,
...
publish_generation_15() {
    local g15_dir=$1 destination=$ARTIFACT_ROOT/generation-15
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 15 "$g15_dir" "$G15_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g15_dir/release-authority-v2.json" \
        "$g15_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g15_dir/syntaur-build-authority-provision" \
        "$g15_dir/syntaur-ship-linux-x86_64" \
        "$g15_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G15_WORKFLOW_COMMIT" \
...
[[ -n $g14_dir && -n $g15_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g14_dir" "$g15_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G14-G15 signed successor verified: generation=15 manifest_sha256=%s\n' \
        "$G15_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G14_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G14'
...
if [[ $receipt_present == true \
    ...
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        ...
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G15_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G14-G15 authority recovery already installed: active_generation=15 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G15_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G15 is not complete'
preflight_recovery_temporaries
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G15_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G15_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G15 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G14_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G14 nor complete G15'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G14_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G14 nor complete G15'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G14_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G14'
...
assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_generation_15 "$g15_material"
write_journal generation_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-15/syntaur-ship-linux-x86_64" \
    "$INSTALLED_SHIPPER" "$G15_SHIPPER_SHA256" 1755 \
    "$G14_SHIPPER_SHA256" 1755
...
g15_workflow_sha=$(workflow_file_sha256 "$G15_WORKFLOW_COMMIT")
publish_exact_file \
    "$ARTIFACT_ROOT/generation-15/trusted-workflow-commit" \
    "$AUTHORITY_ROOT/trusted-workflow-commit" \
    "$g15_workflow_sha" 444 "$g14_workflow_sha" 444
write_journal trust_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-15/release-authority-v2.json.cosign.bundle" \
    "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
    "$G15_BUNDLE_SHA256" 444 "$G14_BUNDLE_SHA256" 444
write_journal bundle_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_exact_file \
    "$ARTIFACT_ROOT/generation-15/release-authority-v2.json" \
    "$AUTHORITY_ROOT/release-authority-v2.json" \
    "$G15_MANIFEST_SHA256" 444 "$G14_MANIFEST_SHA256" 444
...
/usr/bin/printf 'G14-G15 authority recovery installed: active_generation=15 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G15_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
