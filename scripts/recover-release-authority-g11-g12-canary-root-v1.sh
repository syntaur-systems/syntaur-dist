...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g11-g12-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g11-g12-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-12-g11-g12-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g11-g12-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g11-g12-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g11-g12-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g11-g12-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g11-g12-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g11-g12-recovery-v1.runtime
...
die() {
    printf 'release authority G11-G12 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g11-g12-canary-root-v1.sh verify \
    --g11-dir DIR --g12-dir DIR

  sudo recover-release-authority-g11-g12-canary-root-v1.sh install \
    --g11-dir DIR --g12-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G11 authority root to
its exact immutable G12 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G12
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g11-g12-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g11-g12-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    ...
    for path in \
        ...
        "$ARTIFACT_ROOT/.generation-11-g10-g11-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g10-g11-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G10-G11 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g11_dir=$1 g12_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G12_MANIFEST_SHA256" \
        || die 'G12 manifest digest was not fixed before publication'
    valid_sha256 "$G12_BUNDLE_SHA256" \
        || die 'G12 bundle digest was not fixed before publication'
    ...
    [[ $g11_dir != "$g12_dir" ]] || die 'G11 and G12 directories must differ'
    [[ $G11_SHIPPER_SHA256 != "$G12_SHIPPER_SHA256" ]] \
        || die 'G11 and G12 shipper digests must differ'
    [[ $G11_PROVISIONER_SHA256 != "$G12_PROVISIONER_SHA256" ]] \
        || die 'G11 and G12 provisioner digests must differ'
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-11|generation-12|release-authority-manifest.sh) ;;
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-11" "$root/generation-12" 0 0 700 400 500
}
...
require_sources_match_snapshot() {
    ...
    validate_material_layout "$g12_dir" 12 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 11 12; do
        if [[ $generation == 11 ]]; then source=$g11_dir; else source=$g12_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g11_dir" "$g12_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
...
validate_complete_installed_g12() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_active_copy 12 "$ARTIFACT_ROOT/generation-12" \
        "$G12_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G12_SHIPPER_SHA256" 1755 || die 'final G12 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G12_PROVISIONER_SHA256" 755 || die 'final G12 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g11_sha=$3 g12_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g11_sha" ]]; then
        /usr/bin/printf '11\n'
    elif [[ $digest == "$g12_sha" ]]; then
        /usr/bin/printf '12\n'
    else
...
validate_phase_state() {
    local phase=$1 generation_state shipper_state provisioner_state trust_state
    local bundle_state
    local manifest_state vector g11_workflow_sha g12_workflow_sha
    validate_installed_generation 11 \
        "$RECOVERY_SNAPSHOT/generation-11" "$G11_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-12 || -L $ARTIFACT_ROOT/generation-12 ]]; then
    ...
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G11_SHIPPER_SHA256" \
        "$G12_SHIPPER_SHA256" 'installed shipper')
    provisioner_state=$(exact_generation_for_file \
        "$INSTALLED_PROVISIONER" 755 "$G11_PROVISIONER_SHA256" \
        "$G12_PROVISIONER_SHA256" 'installed provisioner')
    ...
    trust_state=$(exact_generation_for_file \
        ...
        "$g11_workflow_sha" "$g12_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G11_BUNDLE_SHA256" "$G12_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G11_MANIFEST_SHA256" "$G12_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$provisioner_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:11:11:11:11:11|prepared:12:11:11:11:11:11|\
        generation_published:12:11:11:11:11:11|\
        generation_published:12:12:11:11:11:11|\
        shipper_published:12:12:11:11:11:11|\
        shipper_published:12:12:12:11:11:11|\
        provisioner_published:12:12:12:11:11:11|\
        provisioner_published:12:12:12:12:11:11|\
        trust_published:12:12:12:12:11:11|\
        trust_published:12:12:12:12:12:11|\
        bundle_published:12:12:12:12:12:11|\
        bundle_published:12:12:12:12:12:12|\
        manifest_published:12:12:12:12:12:12) ;;
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g11-g12-normal-mutation-fence.v1 \
        ...
        --arg g12_bundle "$G12_BUNDLE_SHA256" \
        --arg g12_workflow "$G12_WORKFLOW_COMMIT" \
        ...
        '{schema:$schema,normal_mutations_blocked:true,
          ...
          previous_provisioner_sha256:$g11_provisioner,
          target_generation:12,target_manifest_sha256:$g12_manifest,
          target_bundle_sha256:$g12_bundle,
          target_workflow_commit:$g12_workflow,
          target_shipper_sha256:$g12_shipper,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g11-g12-recovery.v1 \
        --arg phase "$phase" \
        --arg g11_manifest "$G11_MANIFEST_SHA256" \
        --arg g11_shipper "$G11_SHIPPER_SHA256" \
        --arg g11_provisioner "$G11_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g11-g12-recovery-receipt.v1 \
        --arg previous_manifest "$G11_MANIFEST_SHA256" \
        --arg target_manifest "$G12_MANIFEST_SHA256" \
        --arg target_bundle "$G12_BUNDLE_SHA256" \
        --arg target_workflow "$G12_WORKFLOW_COMMIT" \
        --arg target_shipper "$G12_SHIPPER_SHA256" \
        --arg target_provisioner "$G12_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
...
publish_generation_12() {
    local g12_dir=$1 destination=$ARTIFACT_ROOT/generation-12
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 12 "$g12_dir" "$G12_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g12_dir/release-authority-v2.json" \
        "$g12_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g12_dir/syntaur-build-authority-provision" \
        "$g12_dir/syntaur-ship-linux-x86_64" \
        "$g12_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G12_WORKFLOW_COMMIT" \
...
[[ -n $g11_dir && -n $g12_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g11_dir" "$g12_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G11-G12 signed successor verified: generation=12 manifest_sha256=%s\n' \
        "$G12_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G11_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G11'
...
if [[ $receipt_present == true \
    ...
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        ...
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G12_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G11-G12 authority recovery already installed: active_generation=12 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G12_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G12 is not complete'
preflight_recovery_temporaries
...
prepare_recovery_snapshot \
    "$g11_dir" "$g12_dir" "$operator_uid" "$operator_gid" "$snapshot_resume"
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G12_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G12_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G12 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G11_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G11 nor complete G12'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G11_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G11 nor complete G12'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G11_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G11'
...
assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_generation_12 "$g12_material"
write_journal generation_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-12/syntaur-ship-linux-x86_64" \
    "$INSTALLED_SHIPPER" "$G12_SHIPPER_SHA256" 1755 \
    "$G11_SHIPPER_SHA256" 1755
write_journal shipper_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-12/syntaur-build-authority-provision" \
    "$INSTALLED_PROVISIONER" "$G12_PROVISIONER_SHA256" 755 \
    "$G11_PROVISIONER_SHA256" 755
...
g12_workflow_sha=$(workflow_file_sha256 "$G12_WORKFLOW_COMMIT")
publish_exact_file \
    "$ARTIFACT_ROOT/generation-12/trusted-workflow-commit" \
    "$AUTHORITY_ROOT/trusted-workflow-commit" \
    "$g12_workflow_sha" 444 "$g11_workflow_sha" 444
write_journal trust_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-12/release-authority-v2.json.cosign.bundle" \
    "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
    "$G12_BUNDLE_SHA256" 444 "$G11_BUNDLE_SHA256" 444
write_journal bundle_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_exact_file \
    "$ARTIFACT_ROOT/generation-12/release-authority-v2.json" \
    "$AUTHORITY_ROOT/release-authority-v2.json" \
    "$G12_MANIFEST_SHA256" 444 "$G11_MANIFEST_SHA256" 444
...
/usr/bin/printf 'G11-G12 authority recovery installed: active_generation=12 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G12_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
