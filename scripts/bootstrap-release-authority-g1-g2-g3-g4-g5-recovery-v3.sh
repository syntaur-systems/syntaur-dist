...
readonly G5_AUTHORITY_GIT_TREE=9d4b0d8ee19202eb6e6bc49b5bd402c7ab15a287
readonly G5_AUTHORITY_TREE_SHA256=9cb07095581b128bf6619a4684c2cf110a9710e80dd26f28d25dfdd89eb3c6fc
readonly G5_SOURCE_DATE_EPOCH=1785460246
...
die() {
    printf 'release authority G1-G2-G3-G4-G5 recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  bootstrap-release-authority-g1-g2-g3-g4-g5-recovery-v3.sh verify|stage-g5-build-authority|install \
    --g1-dir DIR --g2-dir DIR --g3-dir DIR --g4-dir DIR --g5-dir DIR \
...
G1 through G5. `verify` is non-mutating. `stage-g5-build-authority`
CAS-installs the exact G5 provisioner and G5 Genesis validator while the
release root remains absent. After external G5 Genesis validation, `install`
atomically publishes the retained G1-G2-G3-G4-G5 chain with G5 active.
EOF
    exit 2
}
...
action=$1
shift
[[ $action == verify || $action == stage-g5-build-authority \
...
for source_dir in "$g1_dir" "$g2_dir" "$g3_dir" "$g4_dir" "$g5_dir"; do
...
validate_release_chain \
    ...
    "$g1_dir" "$g2_dir" "$g3_dir" "$g4_dir" "$g5_dir"

if [[ $action == verify ]]; then
    printf 'G1-G2-G3-G4-G5 recovery chain verified: active_generation=5 manifest_sha256=%s\n' \
        "$G5_MANIFEST_SHA256"
    exit 0
fi
...
snapshot=/run/syntaur-release-authority-g1-g2-g3-g4-g5-recovery.snapshot
if [[ -e $snapshot || -L $snapshot ]]; then
    ...
    [[ $(find "$snapshot" -xdev -print | wc -l) -le 33 ]] \
...
root_stage=/etc/syntaur/.release-authority.recovery-v3-g1-g2-g3-g4-g5
shipper_stage=/usr/local/bin/.syntaur-ship.recovery-v3-g1-g2-g3-g4-g5
provisioner_stage=/opt/.syntaur-build-authority-provision.recovery-v3-g5
validator_stage=/opt/.syntaur-genesis-validator.recovery-v3-g5
...
if [[ $action == stage-g5-build-authority ]]; then
    provisioner_state=
    validator_state=
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G5 staging requires the release root to remain absent'
    if installed_executable_is_exact \
        "$INSTALLED_PROVISIONER" "$G5_PROVISIONER_SHA256"; then
        provisioner_state=g5
    elif installed_executable_is_exact \
        "$INSTALLED_PROVISIONER" "$G4_PROVISIONER_SHA256"; then
        provisioner_state=g4
    else
        die 'installed provisioner is neither the exact G4 predecessor nor G5'
    fi
    if installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G5_SHIPPER_SHA256"; then
        validator_state=g5
    elif installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G4_SHIPPER_SHA256"; then
        validator_state=g4
    else
        die 'Genesis validator is neither the exact G4 predecessor nor G5'
    fi
    if [[ $provisioner_state == g4 ]]; then
        install_exact_executable \
            "$snapshot/generation-5/syntaur-build-authority-provision" \
            "$provisioner_stage" "$INSTALLED_PROVISIONER" \
            "$G5_PROVISIONER_SHA256"
    fi
    if [[ $validator_state == g4 ]]; then
        install_exact_executable \
            "$snapshot/generation-5/syntaur-ship-linux-x86_64" \
            "$validator_stage" "$GENESIS_VALIDATOR" \
            "$G5_SHIPPER_SHA256"
    fi
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G5 staging published the release root'
    printf 'G5 Genesis tools staged: provisioner_sha256=%s validator_sha256=%s release_root=absent\n' \
        "$G5_PROVISIONER_SHA256" "$G5_SHIPPER_SHA256"
    exit 0
fi
...
validate_genesis_evidence() {
    ...
    jq -se \
        ...
        --arg source "$G5_AUTHORITY_COMMIT" \
        --arg source_tree "$G5_AUTHORITY_GIT_TREE" \
        --arg authority_parent_commit "$G4_AUTHORITY_COMMIT" \
        --arg authority_parent_tree "$G4_AUTHORITY_GIT_TREE" \
        ...
        --arg shipper "$G5_SHIPPER_SHA256" \
        --arg provisioner "$G5_PROVISIONER_SHA256" \
        ...
        ' "$evidence" >/dev/null \
        || die 'Genesis evidence does not satisfy the exact G5/G2 contract'
}
...
genesis_receipt_json() {
    jq -cn \
        --arg authority_commit "$G5_AUTHORITY_COMMIT" \
        --arg manifest_sha256 "$G5_MANIFEST_SHA256" \
        --arg evidence_sha256 "$expected_genesis_evidence_sha256" \
        --arg engine_commit "$ENGINE_COMMIT" \
        --arg rustsec_commit "$RUSTSEC_DB_COMMIT" \
        --arg shipper_sha256 "$G5_SHIPPER_SHA256" \
        --arg provisioner_sha256 "$G5_PROVISIONER_SHA256" \
        '{
...
validate_genesis_receipt() {
    ...
    [[ $canonical == "$expected" ]] \
        || die 'Genesis receipt differs from the G5 independent record'
}
...
validate_installed_layout() {
    ...
    for name in release-authority-v2.json \
        ...
        /usr/bin/cmp -s "$AUTHORITY_ROOT/$name" \
            "$AUTHORITY_ROOT/release-authority/generation-5/$name" \
...
preflight_live_shipper() {
    ...
    if [[ $authority_root_exists == true ]] \
        && installed_executable_is_exact \
            "$INSTALLED_SHIPPER" "$G5_SHIPPER_SHA256" 1755; then
        return
    fi
    installed_executable_is_exact \
...
remove_exact_genesis_validator() {
    ...
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G5_SHIPPER_SHA256" \
        || die 'G5 Genesis validator changed before retirement'
...
validate_mac_material
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G5_PROVISIONER_SHA256" \
    || die 'install requires the exact staged G5 provisioner'
if [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]]; then
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G5_SHIPPER_SHA256" \
        || die 'install requires the exact staged G5 Genesis validator'
fi
...
if [[ $authority_root_exists == false ]]; then
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$snapshot/generation-5/release-authority-v2.json.cosign.bundle" \
...
if ! installed_executable_is_exact \
    ...
    install_exact_executable \
        "$snapshot/generation-5/syntaur-ship-linux-x86_64" \
        "$shipper_stage" "$INSTALLED_SHIPPER" "$G5_SHIPPER_SHA256" 1755
fi
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G5_PROVISIONER_SHA256" \
    || die 'active G5 provisioner differs'
run_operator_authority_status
remove_exact_genesis_validator
printf 'G1-G2-G3-G4-G5 authority recovery installed: active_generation=5 manifest_sha256=%s\n' \
    "$G5_MANIFEST_SHA256"
