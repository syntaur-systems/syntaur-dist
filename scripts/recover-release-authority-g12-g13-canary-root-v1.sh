...
readonly G12_MANIFEST_SHA256=786bc8bcaedebe4d56c66aa839cbedb30762d367b2d02cf0b0a00bbc8b8e9526
readonly G12_BUNDLE_SHA256=610498d2823898413625bade1301105a6720892440db6ff23829a76495339e86
readonly G12_WORKFLOW_COMMIT=bcd752c62305e84b3ccb5035e443db127bdbed5a
readonly G12_SHIPPER_SHA256=1d9a88bb83d968d09bab362c095173aab20f8be455d151f0d04081ac210f01d5
readonly G12_VERIFIER_SHA256=4b050fe880b8dbd4a6d41b24e305837a117c9be7b7a21c6b6e55552f5d6f35a5
readonly G12_PROVISIONER_SHA256=315bf5591b1ac00ffb443a8e8003fb77b7a421a5e66e91e6e9fd3ed5f859860b
...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g12-g13-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-13-g12-g13-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g12-g13-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g12-g13-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g12-g13-recovery-v1.runtime
...
die() {
    printf 'release authority G12-G13 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g12-g13-canary-root-v1.sh verify \
    --g12-dir DIR --g13-dir DIR

  sudo recover-release-authority-g12-g13-canary-root-v1.sh install \
    --g12-dir DIR --g13-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G12 authority root to
its exact immutable G13 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G13
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g12-g13-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g12-g13-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    ...
    for path in \
        ...
        "$ARTIFACT_ROOT/.generation-12-g11-g12-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g11-g12-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G11-G12 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g12_dir=$1 g13_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G13_MANIFEST_SHA256" \
        || die 'G13 manifest digest was not fixed before publication'
    valid_sha256 "$G13_BUNDLE_SHA256" \
        || die 'G13 bundle digest was not fixed before publication'
    ...
    [[ $g12_dir != "$g13_dir" ]] || die 'G12 and G13 directories must differ'
    [[ $G12_SHIPPER_SHA256 != "$G13_SHIPPER_SHA256" ]] \
        || die 'G12 and G13 shipper digests must differ'
    [[ $G12_PROVISIONER_SHA256 == "$G13_PROVISIONER_SHA256" ]] \
        || die 'G12 and G13 must retain the exact shared provisioner'
    validate_material "$g12_dir" 12 \
        "$G12_MANIFEST_SHA256" "$G12_BUNDLE_SHA256" \
        "$G12_WORKFLOW_COMMIT" "$G12_SHIPPER_SHA256" \
        "$G12_VERIFIER_SHA256" "$G12_PROVISIONER_SHA256" \
    ...
    "$manifest_helper" assert-successor \
        "$g12_dir/release-authority-v2.json" \
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-12|generation-13|release-authority-manifest.sh) ;;
            *) die 'partial recovery input snapshot has an unexpected entry' ;;
        esac
    done <<<"$actual"
    for generation in 12 13; do
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-12" "$root/generation-13" 0 0 700 400 500
}

require_sources_match_snapshot() {
    local g12_dir=$1 g13_dir=$2 operator_uid=$3 operator_gid=$4
    local generation source name
    validate_material_layout "$g12_dir" 12 \
        "$operator_uid" "$operator_gid" 500 400 500
    validate_material_layout "$g13_dir" 13 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 12 13; do
        if [[ $generation == 12 ]]; then source=$g12_dir; else source=$g13_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g12_dir" "$g13_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
    ...
    /usr/bin/install -d -o root -g root -m 0700 \
        "$RECOVERY_SNAPSHOT_STAGE" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-12" \
        "$RECOVERY_SNAPSHOT_STAGE/generation-13"
    snapshot_release "$g12_dir" "$RECOVERY_SNAPSHOT_STAGE/generation-12"
...
validate_complete_installed_g13() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_fixed_installed_generation 12 \
        "$G12_MANIFEST_SHA256" "$G12_BUNDLE_SHA256" \
        "$G12_WORKFLOW_COMMIT" "$G12_SHIPPER_SHA256" \
        "$G12_VERIFIER_SHA256" "$G12_PROVISIONER_SHA256"
    ...
    validate_active_copy 13 "$ARTIFACT_ROOT/generation-13" \
        "$G13_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G13_SHIPPER_SHA256" 1755 || die 'final G13 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G13_PROVISIONER_SHA256" 755 || die 'final G13 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g12_sha=$3 g13_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g12_sha" ]]; then
        /usr/bin/printf '12\n'
    elif [[ $digest == "$g13_sha" ]]; then
        /usr/bin/printf '13\n'
    else
...
validate_phase_state() {
    ...
    local manifest_state vector g12_workflow_sha g13_workflow_sha
    validate_installed_generation 12 \
        "$RECOVERY_SNAPSHOT/generation-12" "$G12_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-13 || -L $ARTIFACT_ROOT/generation-13 ]]; then
    ...
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G12_SHIPPER_SHA256" \
        "$G13_SHIPPER_SHA256" 'installed shipper')
    g12_workflow_sha=$(workflow_file_sha256 "$G12_WORKFLOW_COMMIT")
    ...
    trust_state=$(exact_generation_for_file \
        ...
        "$g12_workflow_sha" "$g13_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G12_BUNDLE_SHA256" "$G13_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G12_MANIFEST_SHA256" "$G13_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:12:12:12:12|prepared:13:12:12:12:12|\
        generation_published:13:12:12:12:12|\
        generation_published:13:13:12:12:12|\
        shipper_published:13:13:12:12:12|\
        provisioner_published:13:13:12:12:12|\
        provisioner_published:13:13:13:12:12|\
        trust_published:13:13:13:12:12|\
        trust_published:13:13:13:13:12|\
        bundle_published:13:13:13:13:12|\
        bundle_published:13:13:13:13:13|\
        manifest_published:13:13:13:13:13) ;;
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g12-g13-normal-mutation-fence.v1 \
        --arg recovery_journal authority-g12-g13-recovery-v1.json \
        --arg g12_manifest "$G12_MANIFEST_SHA256" \
        --arg g12_shipper "$G12_SHIPPER_SHA256" \
        --arg g12_provisioner "$G12_PROVISIONER_SHA256" \
        --arg g13_manifest "$G13_MANIFEST_SHA256" \
        --arg g13_bundle "$G13_BUNDLE_SHA256" \
        --arg g13_workflow "$G13_WORKFLOW_COMMIT" \
        ...
        '{schema:$schema,normal_mutations_blocked:true,
          ...
          previous_generation:12,previous_manifest_sha256:$g12_manifest,
          previous_shipper_sha256:$g12_shipper,
          previous_provisioner_sha256:$g12_provisioner,
          target_generation:13,target_manifest_sha256:$g13_manifest,
          target_bundle_sha256:$g13_bundle,
          target_workflow_commit:$g13_workflow,
          target_shipper_sha256:$g13_shipper,
          target_provisioner_sha256:$g13_provisioner,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g12-g13-recovery.v1 \
        --arg phase "$phase" \
        --arg g12_manifest "$G12_MANIFEST_SHA256" \
        --arg g12_shipper "$G12_SHIPPER_SHA256" \
        --arg g12_provisioner "$G12_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g12-g13-recovery-receipt.v1 \
        --arg previous_manifest "$G12_MANIFEST_SHA256" \
        --arg target_manifest "$G13_MANIFEST_SHA256" \
        --arg target_bundle "$G13_BUNDLE_SHA256" \
        --arg target_workflow "$G13_WORKFLOW_COMMIT" \
        --arg target_shipper "$G13_SHIPPER_SHA256" \
        --arg target_provisioner "$G13_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,completed:true,
          previous_generation:12,previous_manifest_sha256:$previous_manifest,
          target_generation:13,target_manifest_sha256:$target_manifest,
...
publish_generation_13() {
    local g13_dir=$1 destination=$ARTIFACT_ROOT/generation-13
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 13 "$g13_dir" "$G13_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g13_dir/release-authority-v2.json" \
        "$g13_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g13_dir/syntaur-build-authority-provision" \
        "$g13_dir/syntaur-ship-linux-x86_64" \
        "$g13_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G13_WORKFLOW_COMMIT" \
...
[[ -n $g12_dir && -n $g13_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g12_dir" "$g13_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G12-G13 signed successor verified: generation=13 manifest_sha256=%s\n' \
        "$G13_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G12_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G12'
...
if [[ $receipt_present == true \
    ...
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        ...
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G13_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G12-G13 authority recovery already installed: active_generation=13 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G13_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G13 is not complete'
preflight_recovery_temporaries
...
prepare_recovery_snapshot \
    "$g12_dir" "$g13_dir" "$operator_uid" "$operator_gid" "$snapshot_resume"
g12_material=$RECOVERY_SNAPSHOT/generation-12
g13_material=$RECOVERY_SNAPSHOT/generation-13
installed_executable_is_exact "$INSTALLED_PROVISIONER" \
    "$G13_PROVISIONER_SHA256" 755 \
    || die 'G12/G13 shared provisioner differs before recovery'
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G13_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G13_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G13 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G12_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G12 nor complete G13'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G12_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G12 nor complete G13'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G12_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G12'
    validate_installed_generation 12 "$g12_material" "$G12_WORKFLOW_COMMIT"
    validate_retained_chain 12
    validate_active_copy 12 "$g12_material" "$G12_WORKFLOW_COMMIT"
    run_operator_authority_status "$operator_uid" "$operator_gid"
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'G12 authority status changed product or deployment state'
    assert_locks "$root_lock_identity" "$global_lock_identity" \
        "$deploy_lock_identity"
...
assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_generation_13 "$g13_material"
write_journal generation_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-13/syntaur-ship-linux-x86_64" \
    "$INSTALLED_SHIPPER" "$G13_SHIPPER_SHA256" 1755 \
    "$G12_SHIPPER_SHA256" 1755
...
g13_workflow_sha=$(workflow_file_sha256 "$G13_WORKFLOW_COMMIT")
publish_exact_file \
    "$ARTIFACT_ROOT/generation-13/trusted-workflow-commit" \
    "$AUTHORITY_ROOT/trusted-workflow-commit" \
    "$g13_workflow_sha" 444 "$g12_workflow_sha" 444
write_journal trust_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-13/release-authority-v2.json.cosign.bundle" \
    "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
    "$G13_BUNDLE_SHA256" 444 "$G12_BUNDLE_SHA256" 444
write_journal bundle_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_exact_file \
    "$ARTIFACT_ROOT/generation-13/release-authority-v2.json" \
    "$AUTHORITY_ROOT/release-authority-v2.json" \
    "$G13_MANIFEST_SHA256" 444 "$G12_MANIFEST_SHA256" 444
...
/usr/bin/printf 'G12-G13 authority recovery installed: active_generation=13 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G13_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
