...
readonly RECOVERY_JOURNAL=$AUTHORITY_ROOT/authority-g16-g17-recovery-v1.json
readonly RECOVERY_JOURNAL_TEMP=$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.json.tmp
readonly RECOVERY_STAGE=$ARTIFACT_ROOT/.generation-17-g16-g17-recovery-v1.staged
readonly RECOVERY_SNAPSHOT=$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.inputs
readonly RECOVERY_SNAPSHOT_STAGE=$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.inputs.staged
readonly RECOVERY_SNAPSHOT_RETIRED=$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.inputs.retiring
readonly RECOVERY_FENCE_TEMP=$AUTHORITY_ROOT/.authority-g16-g17-recovery-v1.fence.tmp
readonly RECOVERY_RECEIPT=/etc/syntaur/release-authority-g16-g17-recovery-v1.receipt.json
...
readonly SEALED_RUNTIME_ROOT=/etc/syntaur/release-authority-g16-g17-recovery-v1.runtime
...
die() {
    printf 'release authority G16-G17 active-root recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  recover-release-authority-g16-g17-canary-root-v1.sh verify \
    --g16-dir DIR --g17-dir DIR

  sudo recover-release-authority-g16-g17-canary-root-v1.sh install \
    --g16-dir DIR --g17-dir DIR \
    --expected-current-shipper-sha256 HEX

This fixed recovery moves one already-installed exact G16 authority root to
its exact immutable G17 successor. It does not touch product source, release
state, TrueNAS, cameras, NVR, or Frame. Installation publishes the active G17
...
require_sealed_runtime() {
    ...
    expected=$(/usr/bin/printf '%s\n' \
        recover-release-authority-g16-g17-canary-root-v1.sh \
...
publication_temporary_path() {
    ...
    /usr/bin/printf '%s/.%s.g16-g17-recovery-v1\n' \
...
require_no_predecessor_recovery_transients() {
    local path
    for path in \
        "$AUTHORITY_ROOT/authority-g15-g16-recovery-v1.json" \
        "$AUTHORITY_ROOT/.authority-g15-g16-recovery-v1.json.tmp" \
        "$AUTHORITY_ROOT/.authority-g15-g16-recovery-v1.inputs" \
        "$AUTHORITY_ROOT/.authority-g15-g16-recovery-v1.inputs.staged" \
        "$AUTHORITY_ROOT/.authority-g15-g16-recovery-v1.inputs.retiring" \
        "$AUTHORITY_ROOT/.authority-g15-g16-recovery-v1.fence.tmp" \
        "$ARTIFACT_ROOT/.generation-16-g15-g16-recovery-v1.staged" \
        "/etc/syntaur/release-authority-g15-g16-recovery-v1.receipt.json.tmp"; do
        [[ ! -e $path && ! -L $path ]] \
            || die 'predecessor G15-G16 recovery is incomplete'
    done
}
...
verify_release_material() {
    local g16_dir=$1 g17_dir=$2 operator_uid=$3 operator_gid=$4
    local directory_mode=${5:-500} data_mode=${6:-400}
    local executable_mode=${7:-500}
    valid_sha256 "$G17_MANIFEST_SHA256" \
        || die 'G17 manifest digest was not fixed before publication'
    valid_sha256 "$G17_BUNDLE_SHA256" \
        || die 'G17 bundle digest was not fixed before publication'
    ...
    [[ $g16_dir != "$g17_dir" ]] || die 'G16 and G17 directories must differ'
    [[ $G16_SHIPPER_SHA256 != "$G17_SHIPPER_SHA256" ]] \
        || die 'G16 and G17 shipper digests must differ'
    [[ $G16_PROVISIONER_SHA256 == "$G17_PROVISIONER_SHA256" ]] \
        || die 'G16 and G17 must retain the exact shared provisioner'
...
discard_snapshot_tree() {
    ...
    while IFS= read -r name; do
        [[ -z $name ]] && continue
        case $name in
            generation-16|generation-17|release-authority-manifest.sh) ;;
            *) die 'partial recovery input snapshot has an unexpected entry' ;;
        esac
    done <<<"$actual"
    for generation in 16 17; do
...
validate_recovery_snapshot() {
    ...
    verify_release_material \
        "$root/generation-16" "$root/generation-17" 0 0 700 400 500
}
...
require_sources_match_snapshot() {
    ...
    validate_material_layout "$g17_dir" 17 \
        "$operator_uid" "$operator_gid" 500 400 500
    for generation in 16 17; do
        if [[ $generation == 16 ]]; then source=$g16_dir; else source=$g17_dir; fi
        for name in release-authority-v2.json \
            release-authority-v2.json.cosign.bundle \
            syntaur-build-authority-provision \
            syntaur-ship-linux-x86_64 syntaur-verify-linux-x86_64; do
...
prepare_recovery_snapshot() {
    ...
    if [[ -e $RECOVERY_SNAPSHOT || -L $RECOVERY_SNAPSHOT ]]; then
        ...
        if [[ $resume != true ]]; then
            require_sources_match_snapshot \
                "$g16_dir" "$g17_dir" "$operator_uid" "$operator_gid"
        fi
        return 0
    fi
    [[ $resume != true ]] \
...
validate_complete_installed_g17() {
    require_regular "$COSIGN" 0 0 755 1 'pinned Cosign binary'
    [[ $(sha256_file "$COSIGN") == "$COSIGN_SHA256" ]] \
        || die 'pinned Cosign binary digest differs'
    ...
    validate_active_copy 17 "$ARTIFACT_ROOT/generation-17" \
        "$G17_WORKFLOW_COMMIT"
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G17_SHIPPER_SHA256" 1755 || die 'final G17 shipper differs'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G17_PROVISIONER_SHA256" 755 || die 'final G17 provisioner differs'
}

exact_generation_for_file() {
    local path=$1 mode=$2 g16_sha=$3 g17_sha=$4 label=$5 digest
    require_regular "$path" 0 0 "$mode" 1 "$label"
    digest=$(sha256_file "$path")
    if [[ $digest == "$g16_sha" ]]; then
        /usr/bin/printf '16\n'
    elif [[ $digest == "$g17_sha" ]]; then
        /usr/bin/printf '17\n'
    else
...
validate_phase_state() {
    ...
    local manifest_state vector g16_workflow_sha g17_workflow_sha
    validate_installed_generation 16 \
        "$RECOVERY_SNAPSHOT/generation-16" "$G16_WORKFLOW_COMMIT"
    if [[ -e $ARTIFACT_ROOT/generation-17 || -L $ARTIFACT_ROOT/generation-17 ]]; then
    ...
    shipper_state=$(exact_generation_for_file \
        "$INSTALLED_SHIPPER" 1755 "$G16_SHIPPER_SHA256" \
        "$G17_SHIPPER_SHA256" 'installed shipper')
    ...
    trust_state=$(exact_generation_for_file \
        ...
        "$g16_workflow_sha" "$g17_workflow_sha" 'active workflow trust')
    bundle_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" 444 \
        "$G16_BUNDLE_SHA256" "$G17_BUNDLE_SHA256" 'active authority bundle')
    manifest_state=$(exact_generation_for_file \
        "$AUTHORITY_ROOT/release-authority-v2.json" 444 \
        "$G16_MANIFEST_SHA256" "$G17_MANIFEST_SHA256" 'active authority manifest')
    vector=$generation_state:$shipper_state:$trust_state:$bundle_state:$manifest_state
    case $phase:$vector in
        prepared:0:16:16:16:16|prepared:17:16:16:16:16|\
        generation_published:17:16:16:16:16|\
        generation_published:17:17:16:16:16|\
        shipper_published:17:17:16:16:16|\
        provisioner_published:17:17:16:16:16|\
        provisioner_published:17:17:17:16:16|\
        trust_published:17:17:17:16:16|\
        trust_published:17:17:17:17:16|\
        bundle_published:17:17:17:17:16|\
        bundle_published:17:17:17:17:17|\
        manifest_published:17:17:17:17:17) ;;
...
recovery_fence_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g16-g17-normal-mutation-fence.v1 \
        --arg recovery_journal authority-g16-g17-recovery-v1.json \
        ...
        --arg g17_bundle "$G17_BUNDLE_SHA256" \
        --arg g17_workflow "$G17_WORKFLOW_COMMIT" \
        ...
        '{schema:$schema,normal_mutations_blocked:true,
          ...
          previous_generation:16,previous_manifest_sha256:$g16_manifest,
          previous_shipper_sha256:$g16_shipper,
          previous_provisioner_sha256:$g16_provisioner,
          target_generation:17,target_manifest_sha256:$g17_manifest,
          target_bundle_sha256:$g17_bundle,
          target_workflow_commit:$g17_workflow,
          target_shipper_sha256:$g17_shipper,
          target_provisioner_sha256:$g17_provisioner,
...
journal_json() {
    ...
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g16-g17-recovery.v1 \
        --arg phase "$phase" \
        --arg g16_manifest "$G16_MANIFEST_SHA256" \
        --arg g16_shipper "$G16_SHIPPER_SHA256" \
        --arg g16_provisioner "$G16_PROVISIONER_SHA256" \
...
recovery_receipt_json() {
    local product_digest=$1
    /usr/bin/jq -cjn \
        --arg schema syntaur.authority-g16-g17-recovery-receipt.v1 \
        --arg previous_manifest "$G16_MANIFEST_SHA256" \
        --arg target_manifest "$G17_MANIFEST_SHA256" \
        --arg target_bundle "$G17_BUNDLE_SHA256" \
        --arg target_workflow "$G17_WORKFLOW_COMMIT" \
        --arg target_shipper "$G17_SHIPPER_SHA256" \
        --arg target_provisioner "$G17_PROVISIONER_SHA256" \
        --arg product_state "$product_digest" \
        '{schema:$schema,completed:true,
          previous_generation:16,previous_manifest_sha256:$previous_manifest,
          target_generation:17,target_manifest_sha256:$target_manifest,
...
publish_generation_16() {
    local g17_dir=$1 destination=$ARTIFACT_ROOT/generation-17
    if [[ -e $destination || -L $destination ]]; then
        validate_installed_generation 17 "$g17_dir" "$G17_WORKFLOW_COMMIT"
        return 0
    fi
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$g17_dir/release-authority-v2.json" \
        "$g17_dir/release-authority-v2.json.cosign.bundle" \
        "$RECOVERY_STAGE/"
    /usr/bin/install -o root -g root -m 0555 \
        "$g17_dir/syntaur-build-authority-provision" \
        "$g17_dir/syntaur-ship-linux-x86_64" \
        "$g17_dir/syntaur-verify-linux-x86_64" \
        "$RECOVERY_STAGE/"
    /usr/bin/printf '%s\n' "$G17_WORKFLOW_COMMIT" \
...
[[ -n $g16_dir && -n $g17_dir ]] || usage

if [[ $action == verify ]]; then
    [[ -z $expected_current_shipper_sha256 ]] || usage
    verify_release_material "$g16_dir" "$g17_dir" "$(/usr/bin/id -u)" "$(/usr/bin/id -g)"
    /usr/bin/printf 'G16-G17 signed successor verified: generation=17 manifest_sha256=%s\n' \
        "$G17_MANIFEST_SHA256"
    exit 0
fi
...
require_sealed_runtime
[[ $expected_current_shipper_sha256 == "$G16_SHIPPER_SHA256" ]] \
    || die 'independently recorded current shipper is not exact G16'
...
if [[ $receipt_present == true \
    ...
    && installed_executable_is_exact "$INSTALLED_SHIPPER" \
        ...
        && installed_executable_is_exact "$INSTALLED_PROVISIONER" \
            "$G17_PROVISIONER_SHA256" 755; then
    ...
    [[ $(product_state_digest) == "$product_digest" ]] \
        || die 'completed recovery cleanup changed product or deployment state'
    /usr/bin/printf 'G16-G17 authority recovery already installed: active_generation=17 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
        "$G17_MANIFEST_SHA256" "$transaction_product_digest" \
        "$product_digest"
    exit 0
fi

[[ $receipt_present == false ]] \
    || die 'recovery receipt exists while exact G17 is not complete'
preflight_recovery_temporaries
...
if [[ $journal_present == false ]]; then
    if [[ $active_manifest == "$G17_MANIFEST_SHA256" ]] \
        && installed_executable_is_exact "$INSTALLED_SHIPPER" \
            "$G17_SHIPPER_SHA256" 1755; then
        die 'complete un-journaled G17 is missing its recovery receipt'
    fi
    [[ $active_manifest == "$G16_MANIFEST_SHA256" ]] \
        || die 'un-journaled active manifest is neither exact G16 nor complete G17'
    installed_executable_is_exact "$INSTALLED_SHIPPER" \
        "$G16_SHIPPER_SHA256" 1755 \
        || die 'un-journaled authority state is neither exact G16 nor complete G17'
    installed_executable_is_exact "$INSTALLED_PROVISIONER" \
        "$G16_PROVISIONER_SHA256" 755 \
        || die 'un-journaled authority provisioner is not exact G16'
...
assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_generation_16 "$g17_material"
write_journal generation_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-17/syntaur-ship-linux-x86_64" \
    "$INSTALLED_SHIPPER" "$G17_SHIPPER_SHA256" 1755 \
    "$G16_SHIPPER_SHA256" 1755
...
g17_workflow_sha=$(workflow_file_sha256 "$G17_WORKFLOW_COMMIT")
publish_exact_file \
    "$ARTIFACT_ROOT/generation-17/trusted-workflow-commit" \
    "$AUTHORITY_ROOT/trusted-workflow-commit" \
    "$g17_workflow_sha" 444 "$g16_workflow_sha" 444
write_journal trust_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

publish_exact_file \
    "$ARTIFACT_ROOT/generation-17/release-authority-v2.json.cosign.bundle" \
    "$AUTHORITY_ROOT/release-authority-v2.json.cosign.bundle" \
    "$G17_BUNDLE_SHA256" 444 "$G16_BUNDLE_SHA256" 444
write_journal bundle_published "$transaction_product_digest"
validate_current_journal_state "$transaction_product_digest"

assert_locks "$root_lock_identity" "$global_lock_identity" "$deploy_lock_identity"
publish_exact_file \
    "$ARTIFACT_ROOT/generation-17/release-authority-v2.json" \
    "$AUTHORITY_ROOT/release-authority-v2.json" \
    "$G17_MANIFEST_SHA256" 444 "$G16_MANIFEST_SHA256" 444
...
/usr/bin/printf 'G16-G17 authority recovery installed: active_generation=17 manifest_sha256=%s transaction_product_state_sha256=%s current_product_state_sha256=%s\n' \
    "$G17_MANIFEST_SHA256" "$transaction_product_digest" \
    "$transaction_product_digest"
