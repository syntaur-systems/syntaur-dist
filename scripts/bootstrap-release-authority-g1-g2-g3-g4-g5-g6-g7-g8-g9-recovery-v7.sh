...
readonly G8_AUTHORITY_GIT_TREE=4bd00444e83dc3b3d19fbe3e2d52a6b5048c95b0
...
readonly G9_SOURCE_DATE_EPOCH=1785494366
...
die() {
    printf 'release authority G1-G2-G3-G4-G5-G6-G7-G8-G9 recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  bootstrap-release-authority-g1-g2-g3-g4-g5-g6-g7-g8-g9-recovery-v7.sh verify|stage-g9-build-authority|install \
    --g1-dir DIR --g2-dir DIR --g3-dir DIR --g4-dir DIR --g5-dir DIR --g6-dir DIR --g7-dir DIR --g8-dir DIR --g9-dir DIR \
...
G1 through G9. `verify` is non-mutating. `stage-g9-build-authority`
CAS-installs the exact G9 provisioner and G9 Genesis validator while the
release root remains absent. After external G9 Genesis validation, `install`
atomically publishes the retained G1-G2-G3-G4-G5-G6-G7-G8-G9 chain with G9 active.
EOF
    exit 2
}
...
action=$1
shift
[[ $action == verify || $action == stage-g9-build-authority \
...
for source_dir in "$g1_dir" "$g2_dir" "$g3_dir" "$g4_dir" "$g5_dir" \
    "$g6_dir" "$g7_dir" "$g8_dir" "$g9_dir"; do
...
if [[ $action == verify ]]; then
    printf 'G1-G2-G3-G4-G5-G6-G7-G8-G9 recovery chain verified: active_generation=9 manifest_sha256=%s\n' \
        "$G9_MANIFEST_SHA256"
    exit 0
fi
...
snapshot=/run/syntaur-release-authority-g1-g2-g3-g4-g5-g6-g7-g8-g9-recovery.snapshot
if [[ -e $snapshot || -L $snapshot ]]; then
    ...
    [[ $(find "$snapshot" -xdev -print | wc -l) -le 57 ]] \
...
for generation in 1 2 3 4 5 6 7 8 9; do
...
root_stage=/etc/syntaur/.release-authority.recovery-v7-g1-g2-g3-g4-g5-g6-g7-g8-g9
shipper_stage=/usr/local/bin/.syntaur-ship.recovery-v7-g1-g2-g3-g4-g5-g6-g7-g8-g9
provisioner_stage=/opt/.syntaur-build-authority-provision.recovery-v7-g9
validator_stage=/opt/.syntaur-genesis-validator.recovery-v7-g9
...
if [[ $action == stage-g9-build-authority ]]; then
    provisioner_state=
    validator_state=
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G9 staging requires the release root to remain absent'
    if installed_executable_is_exact \
        ...
        "$INSTALLED_PROVISIONER" "$G8_PROVISIONER_SHA256"; then
        provisioner_state=g8
    else
        die 'installed provisioner is neither the exact G8 predecessor nor G9'
    fi
    if installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G9_SHIPPER_SHA256"; then
        validator_state=g9
    elif installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G8_SHIPPER_SHA256"; then
        validator_state=g8
    else
        die 'Genesis validator is neither the exact G8 predecessor nor G9'
    fi
    if [[ $provisioner_state == g8 ]]; then
        install_exact_executable \
            "$snapshot/generation-9/syntaur-build-authority-provision" \
            "$provisioner_stage" "$INSTALLED_PROVISIONER" \
            "$G9_PROVISIONER_SHA256"
    fi
    if [[ $validator_state == g8 ]]; then
        install_exact_executable \
            "$snapshot/generation-9/syntaur-ship-linux-x86_64" \
            "$validator_stage" "$GENESIS_VALIDATOR" \
            "$G9_SHIPPER_SHA256"
    fi
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G9 staging published the release root'
    printf 'G9 Genesis tools staged: provisioner_sha256=%s validator_sha256=%s release_root=absent\n' \
        "$G9_PROVISIONER_SHA256" "$G9_SHIPPER_SHA256"
    exit 0
fi
...
validate_genesis_evidence() {
    ...
    jq -se \
        ...
        --arg source "$G9_AUTHORITY_COMMIT" \
        --arg source_tree "$G9_AUTHORITY_GIT_TREE" \
        --arg authority_parent_commit "$G8_AUTHORITY_COMMIT" \
        --arg authority_parent_tree "$G8_AUTHORITY_GIT_TREE" \
        ...
        --arg shipper "$G9_SHIPPER_SHA256" \
        --arg provisioner "$G9_PROVISIONER_SHA256" \
        ...
        ' "$evidence" >/dev/null \
        || die 'Genesis evidence does not satisfy the exact G9/G2 contract'
}
...
genesis_receipt_json() {
    jq -cn \
        --arg authority_commit "$G9_AUTHORITY_COMMIT" \
        --arg manifest_sha256 "$G9_MANIFEST_SHA256" \
        --arg evidence_sha256 "$expected_genesis_evidence_sha256" \
        --arg engine_commit "$ENGINE_COMMIT" \
        --arg rustsec_commit "$RUSTSEC_DB_COMMIT" \
        --arg shipper_sha256 "$G9_SHIPPER_SHA256" \
        --arg provisioner_sha256 "$G9_PROVISIONER_SHA256" \
        '{
...
validate_genesis_receipt() {
    ...
    [[ $canonical == "$expected" ]] \
        || die 'Genesis receipt differs from the G9 independent record'
}
...
validate_installed_layout() {
    ...
    for name in release-authority-v2.json \
        ...
        /usr/bin/cmp -s "$AUTHORITY_ROOT/$name" \
            "$AUTHORITY_ROOT/release-authority/generation-9/$name" \
...
preflight_live_shipper() {
    ...
    if [[ $authority_root_exists == true ]] \
        && installed_executable_is_exact \
            "$INSTALLED_SHIPPER" "$G9_SHIPPER_SHA256" 1755; then
        return
    fi
    installed_executable_is_exact \
...
remove_exact_genesis_validator() {
    ...
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G9_SHIPPER_SHA256" \
        || die 'G9 Genesis validator changed before retirement'
...
validate_mac_material
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G9_PROVISIONER_SHA256" \
    || die 'install requires the exact staged G9 provisioner'
if [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]]; then
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G9_SHIPPER_SHA256" \
        || die 'install requires the exact staged G9 Genesis validator'
fi
...
if [[ $authority_root_exists == false ]]; then
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$snapshot/generation-9/release-authority-v2.json.cosign.bundle" \
...
if ! installed_executable_is_exact \
    ...
    install_exact_executable \
        "$snapshot/generation-9/syntaur-ship-linux-x86_64" \
        "$shipper_stage" "$INSTALLED_SHIPPER" "$G9_SHIPPER_SHA256" 1755
fi
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G9_PROVISIONER_SHA256" \
    || die 'active G9 provisioner differs'
run_operator_authority_status
remove_exact_genesis_validator
printf 'G1-G2-G3-G4-G5-G6-G7-G8-G9 authority recovery installed: active_generation=9 manifest_sha256=%s\n' \
    "$G9_MANIFEST_SHA256"
