...
readonly G3_AUTHORITY_GIT_TREE=998e1a949928d145b4186c79384c946c927b79ec
...
readonly G4_AUTHORITY_GIT_TREE=2ec45325475f57d72f59012a48e66f717a30b4d6
readonly G4_AUTHORITY_TREE_SHA256=0f180e8213f4e4daf91ece560dbe849a6e3c960ff9ab511acd041d5de91d483a
readonly G4_SOURCE_DATE_EPOCH=1785449195
...
die() {
    printf 'release authority G1-G2-G3-G4 recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  bootstrap-release-authority-g1-g2-g3-g4-recovery-v2.sh verify|stage-g4-build-authority|install \
    --g1-dir DIR --g2-dir DIR --g3-dir DIR --g4-dir DIR \
...
G1 through G4. `verify` is non-mutating. `stage-g4-build-authority`
CAS-installs the exact G4 provisioner and G4 Genesis validator while the
release root remains absent. After external G4 Genesis validation, `install`
atomically publishes the retained G1-G2-G3-G4 chain with G4 active.
EOF
    exit 2
}
...
action=$1
shift
[[ $action == verify || $action == stage-g4-build-authority \
...
for source_dir in "$g1_dir" "$g2_dir" "$g3_dir" "$g4_dir"; do
...
validate_release_chain \
    ...
    "$g1_dir" "$g2_dir" "$g3_dir" "$g4_dir"

if [[ $action == verify ]]; then
    printf 'G1-G2-G3-G4 recovery chain verified: active_generation=4 manifest_sha256=%s\n' \
        "$G4_MANIFEST_SHA256"
    exit 0
fi
...
snapshot=/run/syntaur-release-authority-g1-g2-g3-g4-recovery.snapshot
if [[ -e $snapshot || -L $snapshot ]]; then
    ...
    [[ $(find "$snapshot" -xdev -print | wc -l) -le 32 ]] \
...
root_stage=/etc/syntaur/.release-authority.recovery-v2-g1-g2-g3-g4
shipper_stage=/usr/local/bin/.syntaur-ship.recovery-v2-g1-g2-g3-g4
provisioner_stage=/opt/.syntaur-build-authority-provision.recovery-v2-g4
validator_stage=/opt/.syntaur-genesis-validator.recovery-v2-g4
...
if [[ $action == stage-g4-build-authority ]]; then
    provisioner_state=
    validator_state=
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G4 staging requires the release root to remain absent'
    if installed_executable_is_exact \
        "$INSTALLED_PROVISIONER" "$G4_PROVISIONER_SHA256"; then
        provisioner_state=g4
    elif installed_executable_is_exact \
        "$INSTALLED_PROVISIONER" "$G2_PROVISIONER_SHA256"; then
        provisioner_state=g2
    else
        die 'installed provisioner is neither the exact G2/G3 predecessor nor G4'
    fi
    if installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G4_SHIPPER_SHA256"; then
        validator_state=g4
    elif installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G2_SHIPPER_SHA256"; then
        validator_state=g2
    else
        die 'Genesis validator is neither the exact G2 predecessor nor G4'
    fi
    if [[ $provisioner_state == g2 ]]; then
        install_exact_executable \
            "$snapshot/generation-4/syntaur-build-authority-provision" \
            "$provisioner_stage" "$INSTALLED_PROVISIONER" \
            "$G4_PROVISIONER_SHA256"
    fi
    if [[ $validator_state == g2 ]]; then
        install_exact_executable \
            "$snapshot/generation-4/syntaur-ship-linux-x86_64" \
            "$validator_stage" "$GENESIS_VALIDATOR" \
            "$G4_SHIPPER_SHA256"
    fi
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G4 staging published the release root'
    printf 'G4 Genesis tools staged: provisioner_sha256=%s validator_sha256=%s release_root=absent\n' \
        "$G4_PROVISIONER_SHA256" "$G4_SHIPPER_SHA256"
    exit 0
fi
...
validate_genesis_evidence() {
    ...
    jq -se \
        ...
        --arg source "$G4_AUTHORITY_COMMIT" \
        --arg source_tree "$G4_AUTHORITY_GIT_TREE" \
        --arg authority_parent_commit "$G3_AUTHORITY_COMMIT" \
        --arg authority_parent_tree "$G3_AUTHORITY_GIT_TREE" \
        --arg engine "$ENGINE_COMMIT" \
        --arg engine_tree "$ENGINE_TREE" \
        --arg shipper "$G4_SHIPPER_SHA256" \
        --arg provisioner "$G4_PROVISIONER_SHA256" \
        ...
        --argjson source_epoch "$G4_SOURCE_DATE_EPOCH" \
        --argjson shipper_size "$shipper_size" \
        --slurpfile manifest \
            "$snapshot/generation-4/release-authority-v2.json" '
        ...
        keys_unsorted == [
          ...
          "authority_parent_tree","engine","authority_source_date_epoch","baseline",
        ...
        .build_authority.source_date_epoch ==
          .authority_source_date_epoch and
        ...
        ' "$evidence" >/dev/null \
        || die 'Genesis evidence does not satisfy the exact G4 contract'
}
...
validate_persistent_build_authority() {
    ...
    catalog="$root/catalog/$G4_AUTHORITY_COMMIT-$ENGINE_COMMIT.json"
    [[ -f $catalog && ! -L $catalog ]] \
        || die 'exact G4 persistent catalog is missing'
    [[ $(stat -c '%u:%g:%a:%h' "$catalog") == \
        "0:$operator_gid:440:1" ]] \
        || die 'exact G4 persistent catalog identity differs'
    [[ $(stat -c '%s' "$catalog") -gt 0 \
        && $(stat -c '%s' "$catalog") -le 65536 ]] \
        || die 'exact G4 persistent catalog size is invalid'
    [[ $(wc -l <"$catalog") -eq 1 ]] \
...
genesis_receipt_json() {
    jq -cn \
        --arg authority_commit "$G4_AUTHORITY_COMMIT" \
        --arg manifest_sha256 "$G4_MANIFEST_SHA256" \
        --arg evidence_sha256 "$expected_genesis_evidence_sha256" \
        --arg engine_commit "$ENGINE_COMMIT" \
        --arg rustsec_commit "$RUSTSEC_DB_COMMIT" \
        --arg shipper_sha256 "$G4_SHIPPER_SHA256" \
        --arg provisioner_sha256 "$G4_PROVISIONER_SHA256" \
        '{
...
validate_genesis_receipt() {
    ...
    [[ $canonical == "$expected" ]] \
        || die 'Genesis receipt differs from the G4 independent record'
}
...
validate_installed_layout() {
    ...
    for name in release-authority-v2.json \
        ...
        /usr/bin/cmp -s "$AUTHORITY_ROOT/$name" \
            "$AUTHORITY_ROOT/release-authority/generation-4/$name" \
...
preflight_live_shipper() {
    ...
    if [[ $authority_root_exists == true ]] \
        && installed_executable_is_exact \
            "$INSTALLED_SHIPPER" "$G4_SHIPPER_SHA256" 1755; then
        return
    fi
    installed_executable_is_exact \
...
remove_exact_genesis_validator() {
    ...
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G4_SHIPPER_SHA256" \
        || die 'G4 Genesis validator changed before retirement'
...
validate_mac_material
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G4_PROVISIONER_SHA256" \
    || die 'install requires the exact staged G4 provisioner'
if [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]]; then
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G4_SHIPPER_SHA256" \
        || die 'install requires the exact staged G4 Genesis validator'
fi
...
if [[ $authority_root_exists == false ]]; then
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$snapshot/generation-4/release-authority-v2.json.cosign.bundle" \
...
if ! installed_executable_is_exact \
    ...
    install_exact_executable \
        "$snapshot/generation-4/syntaur-ship-linux-x86_64" \
        "$shipper_stage" "$INSTALLED_SHIPPER" "$G4_SHIPPER_SHA256" 1755
fi
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G4_PROVISIONER_SHA256" \
    || die 'active G4 provisioner differs'
run_operator_authority_status
remove_exact_genesis_validator
printf 'G1-G2-G3-G4 authority recovery installed: active_generation=4 manifest_sha256=%s\n' \
    "$G4_MANIFEST_SHA256"
