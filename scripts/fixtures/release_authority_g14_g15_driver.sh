...
operator_uid=1000
operator_gid=1000
scenario=${G14_G15_FIXTURE_SCENARIO:-normal}
helper=/bootstrap/release-authority-manifest.sh
source_script=/bootstrap/recover-release-authority-g14-g15-canary-root-v1.sh.source
...
runtime=/etc/syntaur/release-authority-g14-g15-recovery-v1.runtime
recovery=$runtime/recover-release-authority-g14-g15-canary-root-v1.sh
verify_root=/home/sean/recovery-verify
verify_script=$verify_root/recover-release-authority-g14-g15-canary-root-v1.sh

die() {
    printf 'G14-G15 recovery fixture error: %s\n' "$*" >&2
    exit 1
}
...
g15_shipper=$material_root/shipper-g15
printf '#!/usr/bin/bash\nset -euo pipefail\nexit 0\n# shared provisioner\n' \
    >"$provisioner"
...
int main(int argc, char **argv) {
    ...
    const char *generation_lock_marker =
        GENERATION == 14 ? "/run/syntaur-replace-deploy-lock-g14" :
            "/run/syntaur-replace-deploy-lock-g15";
...
/usr/bin/cc -O2 -Wall -Wextra -Werror -DGENERATION=15 \
    /run/fixture-shipper.c -o "$g15_shipper"
chmod 0500 "$provisioner" "$verifier" "$g14_shipper" "$g15_shipper"

provisioner_sha=$(sha256_file "$provisioner")
g14_provisioner_sha=$provisioner_sha
g15_provisioner_sha=$provisioner_sha
verifier_sha=$(sha256_file "$verifier")
g14_shipper_sha=$(sha256_file "$g14_shipper")
g15_shipper_sha=$(sha256_file "$g15_shipper")
...
for generation in $(seq 1 15); do
    ...
    if (( generation == 15 )); then
        install -o "$operator_uid" -g "$operator_gid" -m 0500 \
            "$g15_shipper" "$material/syntaur-ship-linux-x86_64"
        shipper_sha=$g15_shipper_sha
    else
        install -o "$operator_uid" -g "$operator_gid" -m 0500 \
...
g15=$material_root/generation-15
...
g15_manifest_sha=$(sha256_file "$g15/release-authority-v2.json")
g15_bundle_sha=$(sha256_file "$g15/release-authority-v2.json.cosign.bundle")
g14_workflow=$(printf '%040x' 14)
g15_workflow=$(printf '%040x' 15)
cosign_sha=$(sha256_file "$fake_cosign")

patched=/run/recover-release-authority-g14-g15-canary-root-v1.sh
sed \
    ...
    -e "s/^readonly G15_SHIPPER_SHA256=.*/readonly G15_SHIPPER_SHA256=$g15_shipper_sha/" \
    -e "s/^readonly G15_VERIFIER_SHA256=.*/readonly G15_VERIFIER_SHA256=$verifier_sha/" \
    -e "s/^readonly G15_PROVISIONER_SHA256=.*/readonly G15_PROVISIONER_SHA256=$g15_provisioner_sha/" \
...
old_runtime=/etc/syntaur/release-authority-g13-g14-recovery-v1.runtime
old_receipt=/etc/syntaur/release-authority-g13-g14-recovery-v1.receipt.json
install -d -o root -g root -m 0700 "$old_runtime"
printf '%s' 'historical G13-G14 runtime marker' >"$old_runtime/marker"
chown root:root "$old_runtime/marker"
chmod 0500 "$old_runtime/marker"
printf '%s' '{"schema":"syntaur.authority-g13-g14-recovery-receipt.v1","completed":true}' \
    >"$old_receipt"
...
install -o root -g root -m 0444 \
    "$g14/release-authority-v2.json" "$authority_root/release-authority-v2.json"
install -o root -g root -m 0444 \
    "$g14/release-authority-v2.json.cosign.bundle" \
    "$authority_root/release-authority-v2.json.cosign.bundle"
printf '%s\n' "$g14_workflow" >"$authority_root/trusted-workflow-commit"
chown root:root "$authority_root/trusted-workflow-commit"
chmod 0444 "$authority_root/trusted-workflow-commit"
install -o root -g root -m 1755 "$g14/syntaur-ship-linux-x86_64" \
    /usr/local/bin/syntaur-ship
install -o root -g root -m 0755 "$g14/syntaur-build-authority-provision" \
...
verify_exact() {
    setpriv --reuid "$operator_uid" --regid "$operator_gid" --clear-groups \
        /usr/bin/bash "$verify_script" verify --g14-dir "$g14" --g15-dir "$g15"
}

install_exact() {
    local source_g14=${1:-$g14} source_g15=${2:-$g15}
    SUDO_UID=$operator_uid SUDO_GID=$operator_gid \
        "$recovery" install \
        --g14-dir "$source_g14" --g15-dir "$source_g15" \
        --expected-current-shipper-sha256 "$g14_shipper_sha"
}

assert_g14_unchanged() {
    ...
    [[ $(sha256_file /opt/syntaur-build-authority-provision) == \
        "$g14_provisioner_sha" ]]
    [[ ! -e $authority_root/authority-g14-g15-recovery-v1.json ]]
}
...
assert_complete() {
    ...
    [[ $(sha256_file "$authority_root/release-authority-v2.json.cosign.bundle") == \
        "$g15_bundle_sha" ]]
    [[ $(<"$authority_root/trusted-workflow-commit") == "$g15_workflow" ]]
    [[ $(sha256_file /usr/local/bin/syntaur-ship) == "$g15_shipper_sha" ]]
    [[ $(sha256_file /opt/syntaur-build-authority-provision) == \
        "$g15_provisioner_sha" ]]
    [[ $(sha256_file "$old_runtime/marker") == "$old_runtime_marker_sha" ]]
    [[ $(sha256_file "$old_receipt") == "$old_receipt_sha" ]]
    expected=$(seq 1 15 | sed 's/^/generation-/' | LC_ALL=C sort)
    actual=$(find "$artifact_root" -mindepth 1 -maxdepth 1 -printf '%f\n' \
        | LC_ALL=C sort)
    [[ $actual == "$expected" ]]
    [[ $(stat -c '%u:%g:%a:%h' \
        /etc/syntaur/release-authority-g14-g15-recovery-v1.receipt.json) \
        == 0:0:444:1 ]]
    for transient in \
        ...
        "$artifact_root/.generation-15-g14-g15-recovery-v1.staged"; do
...
snapshot_inputs() {
    local snapshot=$authority_root/.authority-g14-g15-recovery-v1.inputs
    local generation source target
    install -d -o root -g root -m 0700 \
        "$snapshot" "$snapshot/generation-14" "$snapshot/generation-15"
...
publish_generation_15_fixture() {
    local installed=$artifact_root/generation-15
    [[ -e $installed ]] && return 0
    install -d -o root -g root -m 0555 "$installed"
    install -o root -g root -m 0444 \
        "$g15/release-authority-v2.json" \
        "$g15/release-authority-v2.json.cosign.bundle" "$installed/"
    printf '%s\n' "$g15_workflow" >"$installed/trusted-workflow-commit"
    chown root:root "$installed/trusted-workflow-commit"
    chmod 0444 "$installed/trusted-workflow-commit"
    install -o root -g root -m 0555 \
        "$g15/syntaur-build-authority-provision" \
        "$g15/syntaur-ship-linux-x86_64" \
        "$g15/syntaur-verify-linux-x86_64" "$installed/"
}

publish_target_shipper_fixture() {
    install -o root -g root -m 1755 \
        "$g15/syntaur-ship-linux-x86_64" /usr/local/bin/syntaur-ship
}
...
publish_target_bundle_fixture() {
    install -o root -g root -m 0444 \
        "$g15/release-authority-v2.json.cosign.bundle" \
        "$authority_root/release-authority-v2.json.cosign.bundle"
}
...
seed_journal() {
    ...
    jq -cjn \
        ...
        --arg g15_manifest "$g15_manifest_sha" \
        --arg g15_bundle "$g15_bundle_sha" \
        --arg g15_workflow "$g15_workflow" \
        --arg g15_shipper "$g15_shipper_sha" \
        --arg g15_provisioner "$g15_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g14-g15-recovery.v1",phase:$phase,
          previous_generation:14,previous_manifest_sha256:$g14_manifest,
          previous_shipper_sha256:$g14_shipper,
          previous_provisioner_sha256:$g14_provisioner,target_generation:15,
          target_manifest_sha256:$g15_manifest,target_bundle_sha256:$g15_bundle,
          target_workflow_commit:$g15_workflow,target_shipper_sha256:$g15_shipper,
          target_provisioner_sha256:$g15_provisioner,product_state_sha256:$product}' \
        >"$authority_root/authority-g14-g15-recovery-v1.json"
    chown root:root "$authority_root/authority-g14-g15-recovery-v1.json"
    chmod 0600 "$authority_root/authority-g14-g15-recovery-v1.json"
}
...
seed_fence() {
    jq -cjn \
        ...
        --arg g15_provisioner "$g15_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g14-g15-normal-mutation-fence.v1",
...
seed_receipt() {
    jq -cjn \
        --arg previous_manifest "$g14_manifest_sha" \
        --arg target_manifest "$g15_manifest_sha" \
        --arg target_bundle "$g15_bundle_sha" \
        --arg target_workflow "$g15_workflow" \
        --arg target_shipper "$g15_shipper_sha" \
        --arg target_provisioner "$g15_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g14-g15-recovery-receipt.v1",completed:true,
...
case $scenario in
    ...
    tamper)
        ...
        expect_failure tampered-digest setpriv --reuid "$operator_uid" \
            ...
            verify --g14-dir "$g14" --g15-dir "$tampered"
        ...
        expect_failure hardlinked-input setpriv --reuid "$operator_uid" \
            --regid "$operator_gid" --clear-groups /usr/bin/bash "$verify_script" \
            verify --g14-dir "$g14" --g15-dir "$hardlinked"
    ...
    journal-without-fence)
        ...
        expect_failure journal-without-fence install_exact \
            /home/sean/removed-g14 /home/sean/removed-g15
        [[ $(product_digest) == "$state_before" ]]
        [[ $(sha256_file "$authority_root/release-authority-v2.json") == \
            "$g14_manifest_sha" ]]
        ;;
    ...
    retirement-crash-no-sources)
        seed_resume_state manifest_published
        seed_receipt
        rm -f "$authority_root/authority-g14-g15-recovery-v1.json"
        mv "$authority_root/.authority-g14-g15-recovery-v1.inputs" \
            "$authority_root/.authority-g14-g15-recovery-v1.inputs.retiring"
        rm -f "$authority_root/.authority-g14-g15-recovery-v1.inputs.retiring/generation-15/syntaur-verify-linux-x86_64"
        install_exact /home/sean/removed-g14 /home/sean/removed-g15
        assert_complete
        ;;
    predecessor-recovery-incomplete)
        printf '%s' '{"schema":"syntaur.authority-g13-g14-recovery.v1"}' \
    ...
    phase-mismatch)
        ...
        install -o root -g root -m 0444 "$g15/release-authority-v2.json" \
            "$authority_root/release-authority-v2.json"
    ...
    status-lock-replace-final)
        ...
        [[ $(jq -er '.phase' \
            ...
            manifest_published ]]
        [[ -e $authority_root/authority-promotion-v1.json ]]
        [[ ! -e /etc/syntaur/release-authority-g14-g15-recovery-v1.receipt.json ]]
        assert_normal_mutation_blocked
        rm -f /run/syntaur-replace-deploy-lock-g15
        install_exact /home/sean/removed-g14 /home/sean/removed-g15
        assert_complete
        ;;
    stale-temporaries)
        : >"$authority_root/.authority-g14-g15-recovery-v1.json.tmp"
        chmod 0600 "$authority_root/.authority-g14-g15-recovery-v1.json.tmp"
        : >"$authority_root/.authority-g14-g15-recovery-v1.fence.tmp"
        chmod 0600 "$authority_root/.authority-g14-g15-recovery-v1.fence.tmp"
        : >/etc/syntaur/release-authority-g14-g15-recovery-v1.receipt.json.tmp
        chmod 0600 \
            /etc/syntaur/release-authority-g14-g15-recovery-v1.receipt.json.tmp
        for destination in \
            /usr/local/bin/syntaur-ship \
            ...
            temporary=$(/usr/bin/dirname "$destination")/.$(/usr/bin/basename "$destination").g14-g15-recovery-v1
            : >"$temporary"
            chmod 0600 "$temporary"
        done
        stage=$artifact_root/.generation-15-g14-g15-recovery-v1.staged
        install -d -o root -g root -m 0700 "$stage"
        install -o root -g root -m 0444 "$g15/release-authority-v2.json" \
            "$stage/release-authority-v2.json"
        chmod 0555 "$stage"
        snapshot_stage=$authority_root/.authority-g14-g15-recovery-v1.inputs.staged
        install -d -o root -g root -m 0700 \
            "$snapshot_stage" "$snapshot_stage/generation-14"
        install -o root -g root -m 0600 "$g14/release-authority-v2.json" \
            "$snapshot_stage/generation-14/release-authority-v2.json"
        install_exact
        assert_complete
        ;;
...
printf 'G14-G15 active-root recovery fixture passed: %s\n' "$scenario"
