...
operator_uid=1000
operator_gid=1000
scenario=${G15_G16_FIXTURE_SCENARIO:-normal}
helper=/bootstrap/release-authority-manifest.sh
source_script=/bootstrap/recover-release-authority-g15-g16-canary-root-v1.sh.source
...
runtime=/etc/syntaur/release-authority-g15-g16-recovery-v1.runtime
recovery=$runtime/recover-release-authority-g15-g16-canary-root-v1.sh
verify_root=/home/sean/recovery-verify
verify_script=$verify_root/recover-release-authority-g15-g16-canary-root-v1.sh

die() {
    printf 'G15-G16 recovery fixture error: %s\n' "$*" >&2
    exit 1
}
...
g16_shipper=$material_root/shipper-g16
printf '#!/usr/bin/bash\nset -euo pipefail\nexit 0\n# shared provisioner\n' \
    >"$provisioner"
...
int main(int argc, char **argv) {
    ...
    const char *generation_lock_marker =
        GENERATION == 15 ? "/run/syntaur-replace-deploy-lock-g15" :
            "/run/syntaur-replace-deploy-lock-g16";
...
/usr/bin/cc -O2 -Wall -Wextra -Werror -DGENERATION=16 \
    /run/fixture-shipper.c -o "$g16_shipper"
chmod 0500 "$provisioner" "$verifier" "$g15_shipper" "$g16_shipper"

provisioner_sha=$(sha256_file "$provisioner")
g15_provisioner_sha=$provisioner_sha
g16_provisioner_sha=$provisioner_sha
verifier_sha=$(sha256_file "$verifier")
g15_shipper_sha=$(sha256_file "$g15_shipper")
g16_shipper_sha=$(sha256_file "$g16_shipper")
...
for generation in $(seq 1 16); do
    ...
    if (( generation == 16 )); then
        install -o "$operator_uid" -g "$operator_gid" -m 0500 \
            "$g16_shipper" "$material/syntaur-ship-linux-x86_64"
        shipper_sha=$g16_shipper_sha
    else
        install -o "$operator_uid" -g "$operator_gid" -m 0500 \
...
g16=$material_root/generation-16
...
g16_manifest_sha=$(sha256_file "$g16/release-authority-v2.json")
g16_bundle_sha=$(sha256_file "$g16/release-authority-v2.json.cosign.bundle")
...
sed \
    ...
    -e "s/^readonly G16_MANIFEST_SHA256=.*/readonly G16_MANIFEST_SHA256=$g16_manifest_sha/" \
    -e "s/^readonly G16_BUNDLE_SHA256=.*/readonly G16_BUNDLE_SHA256=$g16_bundle_sha/" \
    -e "s/^readonly G16_WORKFLOW_COMMIT=.*/readonly G16_WORKFLOW_COMMIT=$g16_workflow/" \
    -e "s/^readonly G16_SHIPPER_SHA256=.*/readonly G16_SHIPPER_SHA256=$g16_shipper_sha/" \
    -e "s/^readonly G16_VERIFIER_SHA256=.*/readonly G16_VERIFIER_SHA256=$verifier_sha/" \
    -e "s/^readonly G16_PROVISIONER_SHA256=.*/readonly G16_PROVISIONER_SHA256=$g16_provisioner_sha/" \
...
old_runtime=/etc/syntaur/release-authority-g14-g15-recovery-v1.runtime
old_receipt=/etc/syntaur/release-authority-g14-g15-recovery-v1.receipt.json
install -d -o root -g root -m 0700 "$old_runtime"
printf '%s' 'historical G14-G15 runtime marker' >"$old_runtime/marker"
chown root:root "$old_runtime/marker"
chmod 0500 "$old_runtime/marker"
printf '%s' '{"schema":"syntaur.authority-g14-g15-recovery-receipt.v1","completed":true}' \
    >"$old_receipt"
...
install -o root -g root -m 0444 \
    "$g15/release-authority-v2.json" "$authority_root/release-authority-v2.json"
install -o root -g root -m 0444 \
    "$g15/release-authority-v2.json.cosign.bundle" \
    "$authority_root/release-authority-v2.json.cosign.bundle"
printf '%s\n' "$g15_workflow" >"$authority_root/trusted-workflow-commit"
chown root:root "$authority_root/trusted-workflow-commit"
chmod 0444 "$authority_root/trusted-workflow-commit"
install -o root -g root -m 1755 "$g15/syntaur-ship-linux-x86_64" \
    /usr/local/bin/syntaur-ship
install -o root -g root -m 0755 "$g15/syntaur-build-authority-provision" \
...
verify_exact() {
    setpriv --reuid "$operator_uid" --regid "$operator_gid" --clear-groups \
        /usr/bin/bash "$verify_script" verify --g15-dir "$g15" --g16-dir "$g16"
}

install_exact() {
    local source_g15=${1:-$g15} source_g16=${2:-$g16}
    SUDO_UID=$operator_uid SUDO_GID=$operator_gid \
        "$recovery" install \
        --g15-dir "$source_g15" --g16-dir "$source_g16" \
        --expected-current-shipper-sha256 "$g15_shipper_sha"
}

assert_g15_unchanged() {
    ...
    [[ $(sha256_file /opt/syntaur-build-authority-provision) == \
        "$g15_provisioner_sha" ]]
    [[ ! -e $authority_root/authority-g15-g16-recovery-v1.json ]]
}
...
assert_complete() {
    ...
    [[ $(sha256_file "$authority_root/release-authority-v2.json.cosign.bundle") == \
        "$g16_bundle_sha" ]]
    [[ $(<"$authority_root/trusted-workflow-commit") == "$g16_workflow" ]]
    [[ $(sha256_file /usr/local/bin/syntaur-ship) == "$g16_shipper_sha" ]]
    [[ $(sha256_file /opt/syntaur-build-authority-provision) == \
        "$g16_provisioner_sha" ]]
    [[ $(sha256_file "$old_runtime/marker") == "$old_runtime_marker_sha" ]]
    [[ $(sha256_file "$old_receipt") == "$old_receipt_sha" ]]
    expected=$(seq 1 16 | sed 's/^/generation-/' | LC_ALL=C sort)
    actual=$(find "$artifact_root" -mindepth 1 -maxdepth 1 -printf '%f\n' \
        | LC_ALL=C sort)
    [[ $actual == "$expected" ]]
    [[ $(stat -c '%u:%g:%a:%h' \
        /etc/syntaur/release-authority-g15-g16-recovery-v1.receipt.json) \
        == 0:0:444:1 ]]
    for transient in \
        ...
        "$artifact_root/.generation-16-g15-g16-recovery-v1.staged"; do
...
snapshot_inputs() {
    local snapshot=$authority_root/.authority-g15-g16-recovery-v1.inputs
    local generation source target
    install -d -o root -g root -m 0700 \
        "$snapshot" "$snapshot/generation-15" "$snapshot/generation-16"
...
publish_generation_16_fixture() {
    local installed=$artifact_root/generation-16
    [[ -e $installed ]] && return 0
    install -d -o root -g root -m 0555 "$installed"
    install -o root -g root -m 0444 \
        "$g16/release-authority-v2.json" \
        "$g16/release-authority-v2.json.cosign.bundle" "$installed/"
    printf '%s\n' "$g16_workflow" >"$installed/trusted-workflow-commit"
    chown root:root "$installed/trusted-workflow-commit"
    chmod 0444 "$installed/trusted-workflow-commit"
    install -o root -g root -m 0555 \
        "$g16/syntaur-build-authority-provision" \
        "$g16/syntaur-ship-linux-x86_64" \
        "$g16/syntaur-verify-linux-x86_64" "$installed/"
}

publish_target_shipper_fixture() {
    install -o root -g root -m 1755 \
        "$g16/syntaur-ship-linux-x86_64" /usr/local/bin/syntaur-ship
}
...
publish_target_bundle_fixture() {
    install -o root -g root -m 0444 \
        "$g16/release-authority-v2.json.cosign.bundle" \
        "$authority_root/release-authority-v2.json.cosign.bundle"
}
...
seed_journal() {
    ...
    jq -cjn \
        ...
        --arg g16_manifest "$g16_manifest_sha" \
        --arg g16_bundle "$g16_bundle_sha" \
        --arg g16_workflow "$g16_workflow" \
        --arg g16_shipper "$g16_shipper_sha" \
        --arg g16_provisioner "$g16_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g15-g16-recovery.v1",phase:$phase,
          previous_generation:15,previous_manifest_sha256:$g15_manifest,
          previous_shipper_sha256:$g15_shipper,
          previous_provisioner_sha256:$g15_provisioner,target_generation:16,
          target_manifest_sha256:$g16_manifest,target_bundle_sha256:$g16_bundle,
          target_workflow_commit:$g16_workflow,target_shipper_sha256:$g16_shipper,
          target_provisioner_sha256:$g16_provisioner,product_state_sha256:$product}' \
        >"$authority_root/authority-g15-g16-recovery-v1.json"
    chown root:root "$authority_root/authority-g15-g16-recovery-v1.json"
    chmod 0600 "$authority_root/authority-g15-g16-recovery-v1.json"
}
...
seed_fence() {
    jq -cjn \
        ...
        --arg g16_provisioner "$g16_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g15-g16-normal-mutation-fence.v1",
...
seed_receipt() {
    jq -cjn \
        --arg previous_manifest "$g15_manifest_sha" \
        --arg target_manifest "$g16_manifest_sha" \
        --arg target_bundle "$g16_bundle_sha" \
        --arg target_workflow "$g16_workflow" \
        --arg target_shipper "$g16_shipper_sha" \
        --arg target_provisioner "$g16_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g15-g16-recovery-receipt.v1",completed:true,
...
case $scenario in
    ...
    tamper)
        ...
        expect_failure tampered-digest setpriv --reuid "$operator_uid" \
            ...
            verify --g15-dir "$g15" --g16-dir "$tampered"
        ...
        expect_failure hardlinked-input setpriv --reuid "$operator_uid" \
            --regid "$operator_gid" --clear-groups /usr/bin/bash "$verify_script" \
            verify --g15-dir "$g15" --g16-dir "$hardlinked"
    ...
    journal-without-fence)
        ...
        expect_failure journal-without-fence install_exact \
            /home/sean/removed-g15 /home/sean/removed-g16
        [[ $(product_digest) == "$state_before" ]]
        [[ $(sha256_file "$authority_root/release-authority-v2.json") == \
            "$g15_manifest_sha" ]]
        ;;
    ...
    retirement-crash-no-sources)
        seed_resume_state manifest_published
        seed_receipt
        rm -f "$authority_root/authority-g15-g16-recovery-v1.json"
        mv "$authority_root/.authority-g15-g16-recovery-v1.inputs" \
            "$authority_root/.authority-g15-g16-recovery-v1.inputs.retiring"
        rm -f "$authority_root/.authority-g15-g16-recovery-v1.inputs.retiring/generation-16/syntaur-verify-linux-x86_64"
        install_exact /home/sean/removed-g15 /home/sean/removed-g16
        assert_complete
        ;;
    predecessor-recovery-incomplete)
        printf '%s' '{"schema":"syntaur.authority-g14-g15-recovery.v1"}' \
    ...
    phase-mismatch)
        ...
        install -o root -g root -m 0444 "$g16/release-authority-v2.json" \
            "$authority_root/release-authority-v2.json"
    ...
    status-lock-replace-final)
        ...
        [[ $(jq -er '.phase' \
            ...
            manifest_published ]]
        [[ -e $authority_root/authority-promotion-v1.json ]]
        [[ ! -e /etc/syntaur/release-authority-g15-g16-recovery-v1.receipt.json ]]
        assert_normal_mutation_blocked
        rm -f /run/syntaur-replace-deploy-lock-g16
        install_exact /home/sean/removed-g15 /home/sean/removed-g16
        assert_complete
        ;;
    stale-temporaries)
        : >"$authority_root/.authority-g15-g16-recovery-v1.json.tmp"
        chmod 0600 "$authority_root/.authority-g15-g16-recovery-v1.json.tmp"
        : >"$authority_root/.authority-g15-g16-recovery-v1.fence.tmp"
        chmod 0600 "$authority_root/.authority-g15-g16-recovery-v1.fence.tmp"
        : >/etc/syntaur/release-authority-g15-g16-recovery-v1.receipt.json.tmp
        chmod 0600 \
            /etc/syntaur/release-authority-g15-g16-recovery-v1.receipt.json.tmp
        for destination in \
            /usr/local/bin/syntaur-ship \
            ...
            temporary=$(/usr/bin/dirname "$destination")/.$(/usr/bin/basename "$destination").g15-g16-recovery-v1
            : >"$temporary"
            chmod 0600 "$temporary"
        done
        stage=$artifact_root/.generation-16-g15-g16-recovery-v1.staged
        install -d -o root -g root -m 0700 "$stage"
        install -o root -g root -m 0444 "$g16/release-authority-v2.json" \
            "$stage/release-authority-v2.json"
        chmod 0555 "$stage"
        snapshot_stage=$authority_root/.authority-g15-g16-recovery-v1.inputs.staged
        install -d -o root -g root -m 0700 \
            "$snapshot_stage" "$snapshot_stage/generation-15"
        install -o root -g root -m 0600 "$g15/release-authority-v2.json" \
            "$snapshot_stage/generation-15/release-authority-v2.json"
        install_exact
        assert_complete
        ;;
...
printf 'G15-G16 active-root recovery fixture passed: %s\n' "$scenario"
