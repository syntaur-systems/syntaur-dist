...
operator_uid=1000
operator_gid=1000
scenario=${G12_G13_FIXTURE_SCENARIO:-normal}
helper=/bootstrap/release-authority-manifest.sh
source_script=/bootstrap/recover-release-authority-g12-g13-canary-root-v1.sh.source
...
runtime=/etc/syntaur/release-authority-g12-g13-recovery-v1.runtime
recovery=$runtime/recover-release-authority-g12-g13-canary-root-v1.sh
verify_root=/home/sean/recovery-verify
verify_script=$verify_root/recover-release-authority-g12-g13-canary-root-v1.sh

die() {
    printf 'G12-G13 recovery fixture error: %s\n' "$*" >&2
    exit 1
}
...
int main(int argc, char **argv) {
    ...
    const char *generation_lock_marker =
        GENERATION == 12 ? "/run/syntaur-replace-deploy-lock-g12" :
            "/run/syntaur-replace-deploy-lock-g13";
...
chmod 0500 "$provisioner" "$verifier" "$g12_shipper" "$g13_shipper"

provisioner_sha=$(sha256_file "$provisioner")
g12_provisioner_sha=$provisioner_sha
...
patched=/run/recover-release-authority-g12-g13-canary-root-v1.sh
sed \
...
old_runtime=/etc/syntaur/release-authority-g11-g12-recovery-v1.runtime
old_receipt=/etc/syntaur/release-authority-g11-g12-recovery-v1.receipt.json
install -d -o root -g root -m 0700 "$old_runtime"
printf '%s' 'historical G11-G12 runtime marker' >"$old_runtime/marker"
chown root:root "$old_runtime/marker"
chmod 0500 "$old_runtime/marker"
printf '%s' '{"schema":"syntaur.authority-g11-g12-recovery-receipt.v1","completed":true}' \
    >"$old_receipt"
...
install -o root -g root -m 0444 \
    "$g12/release-authority-v2.json" "$authority_root/release-authority-v2.json"
install -o root -g root -m 0444 \
    "$g12/release-authority-v2.json.cosign.bundle" \
    "$authority_root/release-authority-v2.json.cosign.bundle"
printf '%s\n' "$g12_workflow" >"$authority_root/trusted-workflow-commit"
chown root:root "$authority_root/trusted-workflow-commit"
chmod 0444 "$authority_root/trusted-workflow-commit"
install -o root -g root -m 1755 "$g12/syntaur-ship-linux-x86_64" \
    /usr/local/bin/syntaur-ship
install -o root -g root -m 0755 "$g12/syntaur-build-authority-provision" \
...
verify_exact() {
    setpriv --reuid "$operator_uid" --regid "$operator_gid" --clear-groups \
        /usr/bin/bash "$verify_script" verify --g12-dir "$g12" --g13-dir "$g13"
}

install_exact() {
    local source_g12=${1:-$g12} source_g13=${2:-$g13}
    SUDO_UID=$operator_uid SUDO_GID=$operator_gid \
        "$recovery" install \
        --g12-dir "$source_g12" --g13-dir "$source_g13" \
        --expected-current-shipper-sha256 "$g12_shipper_sha"
}

assert_g12_unchanged() {
    ...
    [[ $(sha256_file /opt/syntaur-build-authority-provision) == \
        "$g12_provisioner_sha" ]]
    [[ ! -e $authority_root/authority-g12-g13-recovery-v1.json ]]
}
...
assert_complete() {
    ...
    [[ $(sha256_file "$authority_root/release-authority-v2.json.cosign.bundle") == \
        "$g13_bundle_sha" ]]
    [[ $(<"$authority_root/trusted-workflow-commit") == "$g13_workflow" ]]
    ...
    expected=$(seq 1 13 | sed 's/^/generation-/' | LC_ALL=C sort)
    actual=$(find "$artifact_root" -mindepth 1 -maxdepth 1 -printf '%f\n' \
        | LC_ALL=C sort)
    [[ $actual == "$expected" ]]
    [[ $(stat -c '%u:%g:%a:%h' \
        /etc/syntaur/release-authority-g12-g13-recovery-v1.receipt.json) \
        == 0:0:444:1 ]]
    for transient in \
        ...
        "$artifact_root/.generation-13-g12-g13-recovery-v1.staged"; do
...
snapshot_inputs() {
    local snapshot=$authority_root/.authority-g12-g13-recovery-v1.inputs
    local generation source target
    install -d -o root -g root -m 0700 \
        "$snapshot" "$snapshot/generation-12" "$snapshot/generation-13"
...
publish_generation_13_fixture() {
    local installed=$artifact_root/generation-13
    [[ -e $installed ]] && return 0
    install -d -o root -g root -m 0555 "$installed"
    install -o root -g root -m 0444 \
        "$g13/release-authority-v2.json" \
        "$g13/release-authority-v2.json.cosign.bundle" "$installed/"
    printf '%s\n' "$g13_workflow" >"$installed/trusted-workflow-commit"
    chown root:root "$installed/trusted-workflow-commit"
    chmod 0444 "$installed/trusted-workflow-commit"
    install -o root -g root -m 0555 \
        "$g13/syntaur-build-authority-provision" \
        "$g13/syntaur-ship-linux-x86_64" \
        "$g13/syntaur-verify-linux-x86_64" "$installed/"
}

publish_target_shipper_fixture() {
    install -o root -g root -m 1755 \
        "$g13/syntaur-ship-linux-x86_64" /usr/local/bin/syntaur-ship
}
...
seed_journal() {
    ...
    jq -cjn \
        ...
        --arg g13_bundle "$g13_bundle_sha" \
        --arg g13_workflow "$g13_workflow" \
        --arg g13_shipper "$g13_shipper_sha" \
        --arg g13_provisioner "$g13_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g12-g13-recovery.v1",phase:$phase,
          previous_generation:12,previous_manifest_sha256:$g12_manifest,
          previous_shipper_sha256:$g12_shipper,
          previous_provisioner_sha256:$g12_provisioner,target_generation:13,
          target_manifest_sha256:$g13_manifest,target_bundle_sha256:$g13_bundle,
          target_workflow_commit:$g13_workflow,target_shipper_sha256:$g13_shipper,
          target_provisioner_sha256:$g13_provisioner,product_state_sha256:$product}' \
        >"$authority_root/authority-g12-g13-recovery-v1.json"
...
seed_fence() {
    jq -cjn \
        ...
        --arg g13_provisioner "$g13_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g12-g13-normal-mutation-fence.v1",
...
seed_receipt() {
    jq -cjn \
        --arg previous_manifest "$g12_manifest_sha" \
        --arg target_manifest "$g13_manifest_sha" \
        --arg target_bundle "$g13_bundle_sha" \
        --arg target_workflow "$g13_workflow" \
        --arg target_shipper "$g13_shipper_sha" \
        --arg target_provisioner "$g13_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g12-g13-recovery-receipt.v1",completed:true,
...
case $scenario in
    ...
    tamper)
        ...
        expect_failure tampered-digest setpriv --reuid "$operator_uid" \
            ...
            verify --g12-dir "$g12" --g13-dir "$tampered"
        ...
        expect_failure hardlinked-input setpriv --reuid "$operator_uid" \
            --regid "$operator_gid" --clear-groups /usr/bin/bash "$verify_script" \
            verify --g12-dir "$g12" --g13-dir "$hardlinked"
    ...
    journal-without-fence)
        ...
        expect_failure journal-without-fence install_exact \
            /home/sean/removed-g12 /home/sean/removed-g13
        [[ $(product_digest) == "$state_before" ]]
        [[ $(sha256_file "$authority_root/release-authority-v2.json") == \
            "$g12_manifest_sha" ]]
        ;;
    ...
    retirement-crash-no-sources)
        seed_resume_state manifest_published
        seed_receipt
        rm -f "$authority_root/authority-g12-g13-recovery-v1.json"
        mv "$authority_root/.authority-g12-g13-recovery-v1.inputs" \
            "$authority_root/.authority-g12-g13-recovery-v1.inputs.retiring"
        rm -f "$authority_root/.authority-g12-g13-recovery-v1.inputs.retiring/generation-13/syntaur-verify-linux-x86_64"
        install_exact /home/sean/removed-g12 /home/sean/removed-g13
        assert_complete
        ;;
    predecessor-recovery-incomplete)
        printf '%s' '{"schema":"syntaur.authority-g11-g12-recovery.v1"}' \
    ...
    phase-mismatch)
        ...
        install -o root -g root -m 0444 "$g13/release-authority-v2.json" \
            "$authority_root/release-authority-v2.json"
    ...
    status-lock-replace-final)
        ...
        [[ $(jq -er '.phase' \
            ...
            manifest_published ]]
        [[ -e $authority_root/authority-promotion-v1.json ]]
        [[ ! -e /etc/syntaur/release-authority-g12-g13-recovery-v1.receipt.json ]]
        assert_normal_mutation_blocked
        rm -f /run/syntaur-replace-deploy-lock-g13
        install_exact /home/sean/removed-g12 /home/sean/removed-g13
        assert_complete
        ;;
    stale-temporaries)
        : >"$authority_root/.authority-g12-g13-recovery-v1.json.tmp"
        chmod 0600 "$authority_root/.authority-g12-g13-recovery-v1.json.tmp"
        : >"$authority_root/.authority-g12-g13-recovery-v1.fence.tmp"
        chmod 0600 "$authority_root/.authority-g12-g13-recovery-v1.fence.tmp"
        : >/etc/syntaur/release-authority-g12-g13-recovery-v1.receipt.json.tmp
        chmod 0600 \
            /etc/syntaur/release-authority-g12-g13-recovery-v1.receipt.json.tmp
        for destination in \
            /usr/local/bin/syntaur-ship \
            ...
            temporary=$(/usr/bin/dirname "$destination")/.$(/usr/bin/basename "$destination").g12-g13-recovery-v1
            : >"$temporary"
            chmod 0600 "$temporary"
        done
        stage=$artifact_root/.generation-13-g12-g13-recovery-v1.staged
        install -d -o root -g root -m 0700 "$stage"
        install -o root -g root -m 0444 "$g13/release-authority-v2.json" \
            "$stage/release-authority-v2.json"
        chmod 0555 "$stage"
        snapshot_stage=$authority_root/.authority-g12-g13-recovery-v1.inputs.staged
        install -d -o root -g root -m 0700 \
            "$snapshot_stage" "$snapshot_stage/generation-12"
        install -o root -g root -m 0600 "$g12/release-authority-v2.json" \
            "$snapshot_stage/generation-12/release-authority-v2.json"
        install_exact
        assert_complete
        ;;
...
printf 'G12-G13 active-root recovery fixture passed: %s\n' "$scenario"
