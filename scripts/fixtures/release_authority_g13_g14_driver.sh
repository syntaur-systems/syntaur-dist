...
operator_uid=1000
operator_gid=1000
scenario=${G13_G14_FIXTURE_SCENARIO:-normal}
helper=/bootstrap/release-authority-manifest.sh
source_script=/bootstrap/recover-release-authority-g13-g14-canary-root-v1.sh.source
...
runtime=/etc/syntaur/release-authority-g13-g14-recovery-v1.runtime
recovery=$runtime/recover-release-authority-g13-g14-canary-root-v1.sh
verify_root=/home/sean/recovery-verify
verify_script=$verify_root/recover-release-authority-g13-g14-canary-root-v1.sh

die() {
    printf 'G13-G14 recovery fixture error: %s\n' "$*" >&2
    exit 1
}
...
g13_shipper=$material_root/shipper-g13
g14_shipper=$material_root/shipper-g14
printf '#!/usr/bin/bash\nset -euo pipefail\nexit 0\n# shared provisioner\n' \
    >"$provisioner"
...
int main(int argc, char **argv) {
    ...
    const char *generation_lock_marker =
        GENERATION == 13 ? "/run/syntaur-replace-deploy-lock-g13" :
            "/run/syntaur-replace-deploy-lock-g14";
...
/usr/bin/cc -O2 -Wall -Wextra -Werror -DGENERATION=13 \
    /run/fixture-shipper.c -o "$g13_shipper"
/usr/bin/cc -O2 -Wall -Wextra -Werror -DGENERATION=14 \
    /run/fixture-shipper.c -o "$g14_shipper"
chmod 0500 "$provisioner" "$verifier" "$g13_shipper" "$g14_shipper"

provisioner_sha=$(sha256_file "$provisioner")
g13_provisioner_sha=$provisioner_sha
g14_provisioner_sha=$provisioner_sha
verifier_sha=$(sha256_file "$verifier")
g13_shipper_sha=$(sha256_file "$g13_shipper")
g14_shipper_sha=$(sha256_file "$g14_shipper")
...
for generation in $(seq 1 14); do
    ...
    if (( generation == 14 )); then
        install -o "$operator_uid" -g "$operator_gid" -m 0500 \
            "$g14_shipper" "$material/syntaur-ship-linux-x86_64"
        shipper_sha=$g14_shipper_sha
    else
        install -o "$operator_uid" -g "$operator_gid" -m 0500 \
            "$g13_shipper" "$material/syntaur-ship-linux-x86_64"
        shipper_sha=$g13_shipper_sha
    fi
    install -o "$operator_uid" -g "$operator_gid" -m 0500 \
...
g13=$material_root/generation-13
g14=$material_root/generation-14
g13_manifest_sha=$(sha256_file "$g13/release-authority-v2.json")
g13_bundle_sha=$(sha256_file "$g13/release-authority-v2.json.cosign.bundle")
g14_manifest_sha=$(sha256_file "$g14/release-authority-v2.json")
g14_bundle_sha=$(sha256_file "$g14/release-authority-v2.json.cosign.bundle")
g13_workflow=$(printf '%040x' 13)
g14_workflow=$(printf '%040x' 14)
cosign_sha=$(sha256_file "$fake_cosign")
helper_sha=$(sha256_file "$helper")

patched=/run/recover-release-authority-g13-g14-canary-root-v1.sh
sed \
    ...
    -e "s/^readonly G13_MANIFEST_SHA256=.*/readonly G13_MANIFEST_SHA256=$g13_manifest_sha/" \
    -e "s/^readonly G13_BUNDLE_SHA256=.*/readonly G13_BUNDLE_SHA256=$g13_bundle_sha/" \
    -e "s/^readonly G13_WORKFLOW_COMMIT=.*/readonly G13_WORKFLOW_COMMIT=$g13_workflow/" \
    -e "s/^readonly G13_SHIPPER_SHA256=.*/readonly G13_SHIPPER_SHA256=$g13_shipper_sha/" \
    -e "s/^readonly G13_VERIFIER_SHA256=.*/readonly G13_VERIFIER_SHA256=$verifier_sha/" \
    -e "s/^readonly G13_PROVISIONER_SHA256=.*/readonly G13_PROVISIONER_SHA256=$g13_provisioner_sha/" \
    -e "s/^readonly G14_MANIFEST_SHA256=.*/readonly G14_MANIFEST_SHA256=$g14_manifest_sha/" \
    -e "s/^readonly G14_BUNDLE_SHA256=.*/readonly G14_BUNDLE_SHA256=$g14_bundle_sha/" \
    -e "s/^readonly G14_WORKFLOW_COMMIT=.*/readonly G14_WORKFLOW_COMMIT=$g14_workflow/" \
    -e "s/^readonly G14_SHIPPER_SHA256=.*/readonly G14_SHIPPER_SHA256=$g14_shipper_sha/" \
    -e "s/^readonly G14_VERIFIER_SHA256=.*/readonly G14_VERIFIER_SHA256=$verifier_sha/" \
    -e "s/^readonly G14_PROVISIONER_SHA256=.*/readonly G14_PROVISIONER_SHA256=$g14_provisioner_sha/" \
...
old_runtime=/etc/syntaur/release-authority-g12-g13-recovery-v1.runtime
old_receipt=/etc/syntaur/release-authority-g12-g13-recovery-v1.receipt.json
install -d -o root -g root -m 0700 "$old_runtime"
printf '%s' 'historical G12-G13 runtime marker' >"$old_runtime/marker"
chown root:root "$old_runtime/marker"
chmod 0500 "$old_runtime/marker"
printf '%s' '{"schema":"syntaur.authority-g12-g13-recovery-receipt.v1","completed":true}' \
    >"$old_receipt"
...
install -o root -g root -m 0444 \
    "$g13/release-authority-v2.json" "$authority_root/release-authority-v2.json"
install -o root -g root -m 0444 \
    "$g13/release-authority-v2.json.cosign.bundle" \
    "$authority_root/release-authority-v2.json.cosign.bundle"
printf '%s\n' "$g13_workflow" >"$authority_root/trusted-workflow-commit"
chown root:root "$authority_root/trusted-workflow-commit"
chmod 0444 "$authority_root/trusted-workflow-commit"
install -o root -g root -m 1755 "$g13/syntaur-ship-linux-x86_64" \
    /usr/local/bin/syntaur-ship
install -o root -g root -m 0755 "$g13/syntaur-build-authority-provision" \
...
verify_exact() {
    setpriv --reuid "$operator_uid" --regid "$operator_gid" --clear-groups \
        /usr/bin/bash "$verify_script" verify --g13-dir "$g13" --g14-dir "$g14"
}

install_exact() {
    local source_g13=${1:-$g13} source_g14=${2:-$g14}
    SUDO_UID=$operator_uid SUDO_GID=$operator_gid \
        "$recovery" install \
        --g13-dir "$source_g13" --g14-dir "$source_g14" \
        --expected-current-shipper-sha256 "$g13_shipper_sha"
}

assert_g13_unchanged() {
    [[ $(sha256_file "$authority_root/release-authority-v2.json") == \
        "$g13_manifest_sha" ]]
    [[ $(sha256_file /usr/local/bin/syntaur-ship) == "$g13_shipper_sha" ]]
    [[ $(sha256_file /opt/syntaur-build-authority-provision) == \
        "$g13_provisioner_sha" ]]
    [[ ! -e $authority_root/authority-g13-g14-recovery-v1.json ]]
}
...
assert_complete() {
    ...
    [[ $(sha256_file "$authority_root/release-authority-v2.json.cosign.bundle") == \
        "$g14_bundle_sha" ]]
    [[ $(<"$authority_root/trusted-workflow-commit") == "$g14_workflow" ]]
    [[ $(sha256_file /usr/local/bin/syntaur-ship) == "$g14_shipper_sha" ]]
    [[ $(sha256_file /opt/syntaur-build-authority-provision) == \
        "$g14_provisioner_sha" ]]
    [[ $(sha256_file "$old_runtime/marker") == "$old_runtime_marker_sha" ]]
    [[ $(sha256_file "$old_receipt") == "$old_receipt_sha" ]]
    expected=$(seq 1 14 | sed 's/^/generation-/' | LC_ALL=C sort)
    actual=$(find "$artifact_root" -mindepth 1 -maxdepth 1 -printf '%f\n' \
        | LC_ALL=C sort)
    [[ $actual == "$expected" ]]
    [[ $(stat -c '%u:%g:%a:%h' \
        /etc/syntaur/release-authority-g13-g14-recovery-v1.receipt.json) \
        == 0:0:444:1 ]]
    for transient in \
        ...
        "$artifact_root/.generation-14-g13-g14-recovery-v1.staged"; do
...
snapshot_inputs() {
    local snapshot=$authority_root/.authority-g13-g14-recovery-v1.inputs
    local generation source target
    install -d -o root -g root -m 0700 \
        "$snapshot" "$snapshot/generation-13" "$snapshot/generation-14"
...
publish_generation_14_fixture() {
    local installed=$artifact_root/generation-14
    [[ -e $installed ]] && return 0
    install -d -o root -g root -m 0555 "$installed"
    install -o root -g root -m 0444 \
        "$g14/release-authority-v2.json" \
        "$g14/release-authority-v2.json.cosign.bundle" "$installed/"
    printf '%s\n' "$g14_workflow" >"$installed/trusted-workflow-commit"
    chown root:root "$installed/trusted-workflow-commit"
    chmod 0444 "$installed/trusted-workflow-commit"
    install -o root -g root -m 0555 \
        "$g14/syntaur-build-authority-provision" \
        "$g14/syntaur-ship-linux-x86_64" \
        "$g14/syntaur-verify-linux-x86_64" "$installed/"
}

publish_target_shipper_fixture() {
    install -o root -g root -m 1755 \
        "$g14/syntaur-ship-linux-x86_64" /usr/local/bin/syntaur-ship
}
...
publish_target_bundle_fixture() {
    install -o root -g root -m 0444 \
        "$g14/release-authority-v2.json.cosign.bundle" \
        "$authority_root/release-authority-v2.json.cosign.bundle"
}
...
seed_journal() {
    ...
    jq -cjn \
        --arg phase "$phase" \
        --arg g13_manifest "$g13_manifest_sha" \
        --arg g13_shipper "$g13_shipper_sha" \
        --arg g13_provisioner "$g13_provisioner_sha" \
        --arg g14_manifest "$g14_manifest_sha" \
        --arg g14_bundle "$g14_bundle_sha" \
        --arg g14_workflow "$g14_workflow" \
        --arg g14_shipper "$g14_shipper_sha" \
        --arg g14_provisioner "$g14_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g13-g14-recovery.v1",phase:$phase,
          previous_generation:13,previous_manifest_sha256:$g13_manifest,
          previous_shipper_sha256:$g13_shipper,
          previous_provisioner_sha256:$g13_provisioner,target_generation:14,
          target_manifest_sha256:$g14_manifest,target_bundle_sha256:$g14_bundle,
          target_workflow_commit:$g14_workflow,target_shipper_sha256:$g14_shipper,
          target_provisioner_sha256:$g14_provisioner,product_state_sha256:$product}' \
        >"$authority_root/authority-g13-g14-recovery-v1.json"
    chown root:root "$authority_root/authority-g13-g14-recovery-v1.json"
    chmod 0600 "$authority_root/authority-g13-g14-recovery-v1.json"
}
...
seed_fence() {
    jq -cjn \
        ...
        --arg g14_provisioner "$g14_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g13-g14-normal-mutation-fence.v1",
...
seed_receipt() {
    jq -cjn \
        --arg previous_manifest "$g13_manifest_sha" \
        --arg target_manifest "$g14_manifest_sha" \
        --arg target_bundle "$g14_bundle_sha" \
        --arg target_workflow "$g14_workflow" \
        --arg target_shipper "$g14_shipper_sha" \
        --arg target_provisioner "$g14_provisioner_sha" \
        --arg product "$state_before" \
        '{schema:"syntaur.authority-g13-g14-recovery-receipt.v1",completed:true,
...
case $scenario in
    ...
    tamper)
        ...
        expect_failure tampered-digest setpriv --reuid "$operator_uid" \
            ...
            verify --g13-dir "$g13" --g14-dir "$tampered"
        ...
        expect_failure hardlinked-input setpriv --reuid "$operator_uid" \
            --regid "$operator_gid" --clear-groups /usr/bin/bash "$verify_script" \
            verify --g13-dir "$g13" --g14-dir "$hardlinked"
    ...
    journal-without-fence)
        ...
        expect_failure journal-without-fence install_exact \
            /home/sean/removed-g13 /home/sean/removed-g14
        [[ $(product_digest) == "$state_before" ]]
        [[ $(sha256_file "$authority_root/release-authority-v2.json") == \
            "$g13_manifest_sha" ]]
        ;;
    ...
    retirement-crash-no-sources)
        seed_resume_state manifest_published
        seed_receipt
        rm -f "$authority_root/authority-g13-g14-recovery-v1.json"
        mv "$authority_root/.authority-g13-g14-recovery-v1.inputs" \
            "$authority_root/.authority-g13-g14-recovery-v1.inputs.retiring"
        rm -f "$authority_root/.authority-g13-g14-recovery-v1.inputs.retiring/generation-14/syntaur-verify-linux-x86_64"
        install_exact /home/sean/removed-g13 /home/sean/removed-g14
        assert_complete
        ;;
    predecessor-recovery-incomplete)
        printf '%s' '{"schema":"syntaur.authority-g12-g13-recovery.v1"}' \
            >"$authority_root/authority-g12-g13-recovery-v1.json"
        chown root:root "$authority_root/authority-g12-g13-recovery-v1.json"
        chmod 0600 "$authority_root/authority-g12-g13-recovery-v1.json"
        expect_failure predecessor-recovery-incomplete-install install_exact
        assert_g13_unchanged
        ;;
    phase-mismatch)
        seed_resume_state prepared
        install -o root -g root -m 0444 "$g14/release-authority-v2.json" \
            "$authority_root/release-authority-v2.json"
    ...
    status-lock-replace-final)
        ...
        [[ $(jq -er '.phase' \
            ...
            manifest_published ]]
        [[ -e $authority_root/authority-promotion-v1.json ]]
        [[ ! -e /etc/syntaur/release-authority-g13-g14-recovery-v1.receipt.json ]]
        assert_normal_mutation_blocked
        rm -f /run/syntaur-replace-deploy-lock-g14
        install_exact /home/sean/removed-g13 /home/sean/removed-g14
        assert_complete
        ;;
    stale-temporaries)
        : >"$authority_root/.authority-g13-g14-recovery-v1.json.tmp"
        chmod 0600 "$authority_root/.authority-g13-g14-recovery-v1.json.tmp"
        : >"$authority_root/.authority-g13-g14-recovery-v1.fence.tmp"
        chmod 0600 "$authority_root/.authority-g13-g14-recovery-v1.fence.tmp"
        : >/etc/syntaur/release-authority-g13-g14-recovery-v1.receipt.json.tmp
        chmod 0600 \
            /etc/syntaur/release-authority-g13-g14-recovery-v1.receipt.json.tmp
        for destination in \
            /usr/local/bin/syntaur-ship \
            ...
            temporary=$(/usr/bin/dirname "$destination")/.$(/usr/bin/basename "$destination").g13-g14-recovery-v1
            : >"$temporary"
            chmod 0600 "$temporary"
        done
        stage=$artifact_root/.generation-14-g13-g14-recovery-v1.staged
        install -d -o root -g root -m 0700 "$stage"
        install -o root -g root -m 0444 "$g14/release-authority-v2.json" \
            "$stage/release-authority-v2.json"
        chmod 0555 "$stage"
        snapshot_stage=$authority_root/.authority-g13-g14-recovery-v1.inputs.staged
        install -d -o root -g root -m 0700 \
            "$snapshot_stage" "$snapshot_stage/generation-13"
        install -o root -g root -m 0600 "$g13/release-authority-v2.json" \
            "$snapshot_stage/generation-13/release-authority-v2.json"
        install_exact
        assert_complete
        ;;
...
printf 'G13-G14 active-root recovery fixture passed: %s\n' "$scenario"
