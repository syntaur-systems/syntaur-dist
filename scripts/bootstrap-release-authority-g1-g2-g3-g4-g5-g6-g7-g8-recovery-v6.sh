...
readonly G8_SOURCE_DATE_EPOCH=1785480030
...
die() {
    printf 'release authority G1-G2-G3-G4-G5-G6-G7-G8 recovery error: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    /usr/bin/cat >&2 <<'EOF'
Usage:
  bootstrap-release-authority-g1-g2-g3-g4-g5-g6-g7-g8-recovery-v6.sh verify|stage-g8-build-authority|install \
    --g1-dir DIR --g2-dir DIR --g3-dir DIR --g4-dir DIR --g5-dir DIR --g6-dir DIR --g7-dir DIR --g8-dir DIR \
...
G1 through G8. `verify` is non-mutating. `stage-g8-build-authority`
CAS-installs the exact G8 provisioner and G8 Genesis validator while the
release root remains absent. After external G8 Genesis validation, `install`
atomically publishes the retained G1-G2-G3-G4-G5-G6-G7-G8 chain with G8 active.
EOF
    exit 2
}
...
action=$1
shift
[[ $action == verify || $action == stage-g8-build-authority \
...
if [[ $action == verify ]]; then
    printf 'G1-G2-G3-G4-G5-G6-G7-G8 recovery chain verified: active_generation=8 manifest_sha256=%s\n' \
        "$G8_MANIFEST_SHA256"
    exit 0
fi
...
snapshot=/run/syntaur-release-authority-g1-g2-g3-g4-g5-g6-g7-g8-recovery.snapshot
if [[ -e $snapshot || -L $snapshot ]]; then
    ...
    [[ $(find "$snapshot" -xdev -print | wc -l) -le 51 ]] \
...
root_stage=/etc/syntaur/.release-authority.recovery-v6-g1-g2-g3-g4-g5-g6-g7-g8
shipper_stage=/usr/local/bin/.syntaur-ship.recovery-v6-g1-g2-g3-g4-g5-g6-g7-g8
provisioner_stage=/opt/.syntaur-build-authority-provision.recovery-v6-g8
validator_stage=/opt/.syntaur-genesis-validator.recovery-v6-g8
...
if [[ $action == stage-g8-build-authority ]]; then
    provisioner_state=
    validator_state=
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G8 staging requires the release root to remain absent'
    if installed_executable_is_exact \
        ...
        die 'installed provisioner is neither the exact G7 predecessor nor G8'
    fi
    if installed_executable_is_exact \
        ...
        die 'Genesis validator is neither the exact G7 predecessor nor G8'
    fi
    if [[ $provisioner_state == g7 ]]; then
        install_exact_executable \
            "$snapshot/generation-8/syntaur-build-authority-provision" \
            "$provisioner_stage" "$INSTALLED_PROVISIONER" \
            "$G8_PROVISIONER_SHA256"
    fi
    if [[ $validator_state == g7 ]]; then
        install_exact_executable \
            "$snapshot/generation-8/syntaur-ship-linux-x86_64" \
            "$validator_stage" "$GENESIS_VALIDATOR" \
            "$G8_SHIPPER_SHA256"
    fi
    [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]] \
        || die 'G8 staging published the release root'
    printf 'G8 Genesis tools staged: provisioner_sha256=%s validator_sha256=%s release_root=absent\n' \
        "$G8_PROVISIONER_SHA256" "$G8_SHIPPER_SHA256"
    exit 0
fi
...
validate_genesis_evidence() {
    ...
    jq -se \
        ...
        --arg source "$G8_AUTHORITY_COMMIT" \
        --arg source_tree "$G8_AUTHORITY_GIT_TREE" \
        --arg authority_parent_commit "$G7_AUTHORITY_COMMIT" \
        --arg authority_parent_tree "$G7_AUTHORITY_GIT_TREE" \
        ...
        --arg shipper "$G8_SHIPPER_SHA256" \
        --arg provisioner "$G8_PROVISIONER_SHA256" \
        ...
        ' "$evidence" >/dev/null \
        || die 'Genesis evidence does not satisfy the exact G8/G2 contract'
}
...
genesis_receipt_json() {
    jq -cn \
        --arg authority_commit "$G8_AUTHORITY_COMMIT" \
        --arg manifest_sha256 "$G8_MANIFEST_SHA256" \
        --arg evidence_sha256 "$expected_genesis_evidence_sha256" \
        --arg engine_commit "$ENGINE_COMMIT" \
        --arg rustsec_commit "$RUSTSEC_DB_COMMIT" \
        --arg shipper_sha256 "$G8_SHIPPER_SHA256" \
        --arg provisioner_sha256 "$G8_PROVISIONER_SHA256" \
        '{
...
validate_genesis_receipt() {
    ...
    [[ $canonical == "$expected" ]] \
        || die 'Genesis receipt differs from the G8 independent record'
}
...
validate_installed_layout() {
    ...
    for name in release-authority-v2.json \
        ...
        /usr/bin/cmp -s "$AUTHORITY_ROOT/$name" \
            "$AUTHORITY_ROOT/release-authority/generation-8/$name" \
...
preflight_live_shipper() {
    ...
    if [[ $authority_root_exists == true ]] \
        && installed_executable_is_exact \
            "$INSTALLED_SHIPPER" "$G8_SHIPPER_SHA256" 1755; then
        return
    fi
    installed_executable_is_exact \
...
remove_exact_genesis_validator() {
    ...
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G8_SHIPPER_SHA256" \
        || die 'G8 Genesis validator changed before retirement'
...
validate_mac_material
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G8_PROVISIONER_SHA256" \
    || die 'install requires the exact staged G8 provisioner'
if [[ ! -e $AUTHORITY_ROOT && ! -L $AUTHORITY_ROOT ]]; then
    installed_executable_is_exact \
        "$GENESIS_VALIDATOR" "$G8_SHIPPER_SHA256" \
        || die 'install requires the exact staged G8 Genesis validator'
fi
...
if [[ $authority_root_exists == false ]]; then
    ...
    /usr/bin/install -o root -g root -m 0444 \
        "$snapshot/generation-8/release-authority-v2.json.cosign.bundle" \
...
if ! installed_executable_is_exact \
    ...
    install_exact_executable \
        "$snapshot/generation-8/syntaur-ship-linux-x86_64" \
        "$shipper_stage" "$INSTALLED_SHIPPER" "$G8_SHIPPER_SHA256" 1755
fi
installed_executable_is_exact \
    "$INSTALLED_PROVISIONER" "$G8_PROVISIONER_SHA256" \
    || die 'active G8 provisioner differs'
run_operator_authority_status
remove_exact_genesis_validator
printf 'G1-G2-G3-G4-G5-G6-G7-G8 authority recovery installed: active_generation=8 manifest_sha256=%s\n' \
    "$G8_MANIFEST_SHA256"
